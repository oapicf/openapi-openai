...
void chat_completion_response_message_free(chat_completion_response_message_t *chat_completion_response_message) {
    ...
    if(chat_completion_response_message->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "chat_completion_response_message_free");
        return ;
    }
...
cJSON *chat_completion_response_message_convertToJSON(chat_completion_response_message_t *chat_completion_response_message) {
    ...
    if(cJSON_AddStringToObject(item, "role", chat_completion_response_message_role_ToString(chat_completion_response_message->role)) == NULL)
    {
    goto fail; //Enum
    }
