...
static create_embedding_response_t *create_embedding_response_create_internal(
    list_t *data,
    char *model,
    ...
    create_embedding_response_local_var->_library_owned = 1;
    return create_embedding_response_local_var;
}

__attribute__((deprecated)) create_embedding_response_t *create_embedding_response_create(
    list_t *data,
    char *model,
    openai_api_create_embedding_response_OBJECT_e object,
    create_embedding_response_usage_t *usage
    ) {
    return create_embedding_response_create_internal (
        data,
        model,
        object,
        usage
...
void create_embedding_response_free(create_embedding_response_t *create_embedding_response) {
    ...
    if(create_embedding_response->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "create_embedding_response_free");
        return ;
    }
...
cJSON *create_embedding_response_convertToJSON(create_embedding_response_t *create_embedding_response) {
    ...
    if(cJSON_AddStringToObject(item, "object", create_embedding_response_object_ToString(create_embedding_response->object)) == NULL)
    {
    goto fail; //Enum
    }
...
create_embedding_response_t *create_embedding_response_parseFromJSON(cJSON *create_embedding_responseJSON){
    ...
    create_embedding_response_local_var = create_embedding_response_create_internal (
        dataList,
