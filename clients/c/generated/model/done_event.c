...
static done_event_t *done_event_create_internal(
    ...
    done_event_local_var->_library_owned = 1;
...
__attribute__((deprecated)) done_event_t *done_event_create(
    ...
    return done_event_create_internal (
        event,
        data
        );
}
...
void done_event_free(done_event_t *done_event) {
    ...
    if(done_event->_library_owned != 1){
...
cJSON *done_event_convertToJSON(done_event_t *done_event) {
    ...
    if(cJSON_AddStringToObject(item, "event", done_event_event_ToString(done_event->event)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(cJSON_AddStringToObject(item, "data", done_event_data_ToString(done_event->data)) == NULL)
    {
    goto fail; //Enum
    }
...
done_event_t *done_event_parseFromJSON(cJSON *done_eventJSON){
    ...
    done_event_local_var = done_event_create_internal (
        eventVariable,
        dataVariable
        );
