...
static error_event_t *error_event_create_internal(
    ...
    error_event_local_var->_library_owned = 1;
...
__attribute__((deprecated)) error_event_t *error_event_create(
    ...
    return error_event_create_internal (
        event,
        data
        );
}
...
void error_event_free(error_event_t *error_event) {
    ...
    if(error_event->_library_owned != 1){
...
cJSON *error_event_convertToJSON(error_event_t *error_event) {
    ...
    if(cJSON_AddStringToObject(item, "event", error_event_event_ToString(error_event->event)) == NULL)
    {
    goto fail; //Enum
    }
...
error_event_t *error_event_parseFromJSON(cJSON *error_eventJSON){
    ...
    error_event_local_var = error_event_create_internal (
        eventVariable,
        data_local_nonprim
        );
