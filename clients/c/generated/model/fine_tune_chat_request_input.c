...
#include <string.h>
#include <stdio.h>
#include "fine_tune_chat_request_input.h"



static fine_tune_chat_request_input_t *fine_tune_chat_request_input_create_internal(
    list_t *messages,
    list_t *tools,
    int parallel_tool_calls,
    list_t *functions
    ) {
    fine_tune_chat_request_input_t *fine_tune_chat_request_input_local_var = malloc(sizeof(fine_tune_chat_request_input_t));
    if (!fine_tune_chat_request_input_local_var) {
        return NULL;
    }
    fine_tune_chat_request_input_local_var->messages = messages;
    fine_tune_chat_request_input_local_var->tools = tools;
    fine_tune_chat_request_input_local_var->parallel_tool_calls = parallel_tool_calls;
    fine_tune_chat_request_input_local_var->functions = functions;

    fine_tune_chat_request_input_local_var->_library_owned = 1;
    return fine_tune_chat_request_input_local_var;
}
...
__attribute__((deprecated)) fine_tune_chat_request_input_t *fine_tune_chat_request_input_create(
    ...
    return fine_tune_chat_request_input_create_internal (
        messages,
        tools,
        parallel_tool_calls,
        functions
...
void fine_tune_chat_request_input_free(fine_tune_chat_request_input_t *fine_tune_chat_request_input) {
    if(NULL == fine_tune_chat_request_input){
        return ;
    }
    if(fine_tune_chat_request_input->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "fine_tune_chat_request_input_free");
        return ;
    }
    listEntry_t *listEntry;
    if (fine_tune_chat_request_input->messages) {
        list_ForEach(listEntry, fine_tune_chat_request_input->messages) {
            fine_tune_chat_request_input_messages_inner_free(listEntry->data);
        }
        list_freeList(fine_tune_chat_request_input->messages);
        fine_tune_chat_request_input->messages = NULL;
    }
    if (fine_tune_chat_request_input->tools) {
        list_ForEach(listEntry, fine_tune_chat_request_input->tools) {
            chat_completion_tool_free(listEntry->data);
        }
        list_freeList(fine_tune_chat_request_input->tools);
        fine_tune_chat_request_input->tools = NULL;
    }
    if (fine_tune_chat_request_input->functions) {
        list_ForEach(listEntry, fine_tune_chat_request_input->functions) {
            chat_completion_functions_free(listEntry->data);
        }
        list_freeList(fine_tune_chat_request_input->functions);
        fine_tune_chat_request_input->functions = NULL;
    }
    free(fine_tune_chat_request_input);
}

cJSON *fine_tune_chat_request_input_convertToJSON(fine_tune_chat_request_input_t *fine_tune_chat_request_input) {
    cJSON *item = cJSON_CreateObject();

    // fine_tune_chat_request_input->messages
    if(fine_tune_chat_request_input->messages) {
    ...
    if (fine_tune_chat_request_input->messages) {
    list_ForEach(messagesListEntry, fine_tune_chat_request_input->messages) {
    ...
    // fine_tune_chat_request_input->tools
    if(fine_tune_chat_request_input->tools) {
    ...
    if (fine_tune_chat_request_input->tools) {
    list_ForEach(toolsListEntry, fine_tune_chat_request_input->tools) {
    ...
    // fine_tune_chat_request_input->parallel_tool_calls
    if(fine_tune_chat_request_input->parallel_tool_calls) {
    if(cJSON_AddBoolToObject(item, "parallel_tool_calls", fine_tune_chat_request_input->parallel_tool_calls) == NULL) {
    goto fail; //Bool
    }
    }


    // fine_tune_chat_request_input->functions
    if(fine_tune_chat_request_input->functions) {
    ...
    if (fine_tune_chat_request_input->functions) {
    list_ForEach(functionsListEntry, fine_tune_chat_request_input->functions) {
...
fine_tune_chat_request_input_t *fine_tune_chat_request_input_parseFromJSON(cJSON *fine_tune_chat_request_inputJSON){
    ...
    fine_tune_chat_request_input_t *fine_tune_chat_request_input_local_var = NULL;

    // define the local list for fine_tune_chat_request_input->messages
    list_t *messagesList = NULL;

    // define the local list for fine_tune_chat_request_input->tools
    list_t *toolsList = NULL;

    // define the local list for fine_tune_chat_request_input->functions
    list_t *functionsList = NULL;

    // fine_tune_chat_request_input->messages
    cJSON *messages = cJSON_GetObjectItemCaseSensitive(fine_tune_chat_request_inputJSON, "messages");
    ...
    // fine_tune_chat_request_input->tools
    cJSON *tools = cJSON_GetObjectItemCaseSensitive(fine_tune_chat_request_inputJSON, "tools");
    ...
    // fine_tune_chat_request_input->parallel_tool_calls
    cJSON *parallel_tool_calls = cJSON_GetObjectItemCaseSensitive(fine_tune_chat_request_inputJSON, "parallel_tool_calls");
    ...
    // fine_tune_chat_request_input->functions
    cJSON *functions = cJSON_GetObjectItemCaseSensitive(fine_tune_chat_request_inputJSON, "functions");
    ...
    fine_tune_chat_request_input_local_var = fine_tune_chat_request_input_create_internal (
