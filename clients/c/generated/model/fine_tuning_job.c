...
static fine_tuning_job_t *fine_tuning_job_create_internal(
    char *id,
    int created_at,
...
void fine_tuning_job_free(fine_tuning_job_t *fine_tuning_job) {
    ...
    if(fine_tuning_job->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "fine_tuning_job_free");
        return ;
    }
...
cJSON *fine_tuning_job_convertToJSON(fine_tuning_job_t *fine_tuning_job) {
    ...
    if(cJSON_AddStringToObject(item, "object", fine_tuning_job_object_ToString(fine_tuning_job->object)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(cJSON_AddStringToObject(result_files, "", result_filesListEntry->data) == NULL)
    {
        goto fail;
    }
    }
    ...
    if(cJSON_AddStringToObject(item, "status", fine_tuning_job_status_ToString(fine_tuning_job->status)) == NULL)
    {
    goto fail; //Enum
    }
...
fine_tuning_job_t *fine_tuning_job_parseFromJSON(cJSON *fine_tuning_jobJSON){
    ...
    if (cJSON_IsNull(fine_tuned_model)) {
    ...
    if (cJSON_IsNull(organization_id)) {
    ...
    if (cJSON_IsNull(trained_tokens)) {
        trained_tokens = NULL;
    }
    if (!trained_tokens) {
        goto end;
