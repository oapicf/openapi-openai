/*
 * invite_list_response.h
 *
 * 
 */

#ifndef _invite_list_response_H_
#define _invite_list_response_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct invite_list_response_t invite_list_response_t;

#include "invite.h"

// Enum OBJECT for invite_list_response

typedef enum  { openai_api_invite_list_response_OBJECT_NULL = 0, openai_api_invite_list_response_OBJECT_list } openai_api_invite_list_response_OBJECT_e;

char* invite_list_response_object_ToString(openai_api_invite_list_response_OBJECT_e object);

openai_api_invite_list_response_OBJECT_e invite_list_response_object_FromString(char* object);



typedef struct invite_list_response_t {
    openai_api_invite_list_response_OBJECT_e object; //enum
    list_t *data; //nonprimitive container
    char *first_id; // string
    char *last_id; // string
    int has_more; //boolean

    int _library_owned; // Is the library responsible for freeing this object?
} invite_list_response_t;

__attribute__((deprecated)) invite_list_response_t *invite_list_response_create(
    openai_api_invite_list_response_OBJECT_e object,
    list_t *data,
    char *first_id,
    char *last_id,
    int has_more
);

void invite_list_response_free(invite_list_response_t *invite_list_response);

invite_list_response_t *invite_list_response_parseFromJSON(cJSON *invite_list_responseJSON);

cJSON *invite_list_response_convertToJSON(invite_list_response_t *invite_list_response);

#endif /* _invite_list_response_H_ */

