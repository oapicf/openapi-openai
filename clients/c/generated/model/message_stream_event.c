...
static message_stream_event_t *message_stream_event_create_internal(
    ...
    message_stream_event_local_var->_library_owned = 1;
...
__attribute__((deprecated)) message_stream_event_t *message_stream_event_create(
    ...
    return message_stream_event_create_internal (
        event,
        data
        );
}
...
void message_stream_event_free(message_stream_event_t *message_stream_event) {
    ...
    if(message_stream_event->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "message_stream_event_free");
        return ;
    }
...
cJSON *message_stream_event_convertToJSON(message_stream_event_t *message_stream_event) {
    ...
    if(cJSON_AddStringToObject(item, "event", message_stream_event_event_ToString(message_stream_event->event)) == NULL)
    {
    goto fail; //Enum
    }
...
message_stream_event_t *message_stream_event_parseFromJSON(cJSON *message_stream_eventJSON){
    ...
    message_stream_event_local_var = message_stream_event_create_internal (
        eventVariable,
        data_local_nonprim
        );
