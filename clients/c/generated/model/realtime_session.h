/*
 * realtime_session.h
 *
 * Realtime session object configuration.
 */

#ifndef _realtime_session_H_
#define _realtime_session_H_

#include <string.h>
#include "../external/cJSON.h"
#include "../include/list.h"
#include "../include/keyValuePair.h"
#include "../include/binary.h"

typedef struct realtime_session_t realtime_session_t;

#include "realtime_response_create_params_max_response_output_tokens.h"
#include "realtime_response_create_params_tools_inner.h"
#include "realtime_session_input_audio_transcription.h"
#include "realtime_session_model.h"
#include "realtime_session_turn_detection.h"

// Enum MODALITIES for realtime_session

typedef enum  { openai_api_realtime_session_MODALITIES_NULL = 0, openai_api_realtime_session_MODALITIES_text, openai_api_realtime_session_MODALITIES_audio } openai_api_realtime_session_MODALITIES_e;

char* realtime_session_modalities_ToString(openai_api_realtime_session_MODALITIES_e modalities);

openai_api_realtime_session_MODALITIES_e realtime_session_modalities_FromString(char* modalities);

// Enum VOICE for realtime_session

typedef enum  { openai_api_realtime_session_VOICE_NULL = 0, openai_api_realtime_session_VOICE_alloy, openai_api_realtime_session_VOICE_ash, openai_api_realtime_session_VOICE_ballad, openai_api_realtime_session_VOICE_coral, openai_api_realtime_session_VOICE_echo, openai_api_realtime_session_VOICE_sage, openai_api_realtime_session_VOICE_shimmer, openai_api_realtime_session_VOICE_verse } openai_api_realtime_session_VOICE_e;

char* realtime_session_voice_ToString(openai_api_realtime_session_VOICE_e voice);

openai_api_realtime_session_VOICE_e realtime_session_voice_FromString(char* voice);

// Enum INPUTAUDIOFORMAT for realtime_session

typedef enum  { openai_api_realtime_session_INPUTAUDIOFORMAT_NULL = 0, openai_api_realtime_session_INPUTAUDIOFORMAT_pcm16, openai_api_realtime_session_INPUTAUDIOFORMAT_g711_ulaw, openai_api_realtime_session_INPUTAUDIOFORMAT_g711_alaw } openai_api_realtime_session_INPUTAUDIOFORMAT_e;

char* realtime_session_input_audio_format_ToString(openai_api_realtime_session_INPUTAUDIOFORMAT_e input_audio_format);

openai_api_realtime_session_INPUTAUDIOFORMAT_e realtime_session_input_audio_format_FromString(char* input_audio_format);

// Enum OUTPUTAUDIOFORMAT for realtime_session

typedef enum  { openai_api_realtime_session_OUTPUTAUDIOFORMAT_NULL = 0, openai_api_realtime_session_OUTPUTAUDIOFORMAT_pcm16, openai_api_realtime_session_OUTPUTAUDIOFORMAT_g711_ulaw, openai_api_realtime_session_OUTPUTAUDIOFORMAT_g711_alaw } openai_api_realtime_session_OUTPUTAUDIOFORMAT_e;

char* realtime_session_output_audio_format_ToString(openai_api_realtime_session_OUTPUTAUDIOFORMAT_e output_audio_format);

openai_api_realtime_session_OUTPUTAUDIOFORMAT_e realtime_session_output_audio_format_FromString(char* output_audio_format);



typedef struct realtime_session_t {
    char *id; // string
    list_t *modalities; //primitive container
    struct realtime_session_model_t *model; //model
    char *instructions; // string
    openai_api_realtime_session_VOICE_e voice; //enum
    openai_api_realtime_session_INPUTAUDIOFORMAT_e input_audio_format; //enum
    openai_api_realtime_session_OUTPUTAUDIOFORMAT_e output_audio_format; //enum
    struct realtime_session_input_audio_transcription_t *input_audio_transcription; //model
    struct realtime_session_turn_detection_t *turn_detection; //model
    list_t *tools; //nonprimitive container
    char *tool_choice; // string
    double temperature; //numeric
    struct realtime_response_create_params_max_response_output_tokens_t *max_response_output_tokens; //model

    int _library_owned; // Is the library responsible for freeing this object?
} realtime_session_t;

__attribute__((deprecated)) realtime_session_t *realtime_session_create(
    char *id,
    list_t *modalities,
    realtime_session_model_t *model,
    char *instructions,
    openai_api_realtime_session_VOICE_e voice,
    openai_api_realtime_session_INPUTAUDIOFORMAT_e input_audio_format,
    openai_api_realtime_session_OUTPUTAUDIOFORMAT_e output_audio_format,
    realtime_session_input_audio_transcription_t *input_audio_transcription,
    realtime_session_turn_detection_t *turn_detection,
    list_t *tools,
    char *tool_choice,
    double temperature,
    realtime_response_create_params_max_response_output_tokens_t *max_response_output_tokens
);

void realtime_session_free(realtime_session_t *realtime_session);

realtime_session_t *realtime_session_parseFromJSON(cJSON *realtime_sessionJSON);

cJSON *realtime_session_convertToJSON(realtime_session_t *realtime_session);

#endif /* _realtime_session_H_ */

