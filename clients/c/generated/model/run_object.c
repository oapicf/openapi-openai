...
run_object_t *run_object_create(
    char *id,
...
cJSON *run_object_convertToJSON(run_object_t *run_object) {
    ...
    if(cJSON_AddStringToObject(item, "object", objectrun_object_ToString(run_object->object)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(cJSON_AddStringToObject(item, "status", statusrun_object_ToString(run_object->status)) == NULL)
    {
    goto fail; //Enum
    }
...
run_object_t *run_object_parseFromJSON(cJSON *run_objectJSON){
    ...
    run_object_local_var = run_object_create (
