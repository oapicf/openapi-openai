...
static run_step_object_t *run_step_object_create_internal(
    char *id,
    ...
    run_step_object_local_var->_library_owned = 1;
    return run_step_object_local_var;
}

__attribute__((deprecated)) run_step_object_t *run_step_object_create(
    char *id,
    openai_api_run_step_object_OBJECT_e object,
    int created_at,
    char *assistant_id,
    ...
    return run_step_object_create_internal (
        id,
        object,
        created_at,
        assistant_id,
...
void run_step_object_free(run_step_object_t *run_step_object) {
    ...
    if(run_step_object->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "run_step_object_free");
        return ;
    }
...
cJSON *run_step_object_convertToJSON(run_step_object_t *run_step_object) {
    ...
    if(cJSON_AddStringToObject(item, "object", run_step_object_object_ToString(run_step_object->object)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(cJSON_AddStringToObject(item, "type", run_step_object_type_ToString(run_step_object->type)) == NULL)
    {
    goto fail; //Enum
    }
    ...
    if(cJSON_AddStringToObject(item, "status", run_step_object_status_ToString(run_step_object->status)) == NULL)
    {
    goto fail; //Enum
    }
...
run_step_object_t *run_step_object_parseFromJSON(cJSON *run_step_objectJSON){
    ...
    run_step_object_local_var = run_step_object_create_internal (
