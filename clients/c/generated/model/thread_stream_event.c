...
static thread_stream_event_t *thread_stream_event_create_internal(
    ...
    thread_stream_event_local_var->_library_owned = 1;
...
void thread_stream_event_free(thread_stream_event_t *thread_stream_event) {
    ...
    if(thread_stream_event->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "thread_stream_event_free");
        return ;
    }
...
cJSON *thread_stream_event_convertToJSON(thread_stream_event_t *thread_stream_event) {
    ...
    if(cJSON_AddStringToObject(item, "event", thread_stream_event_event_ToString(thread_stream_event->event)) == NULL)
    {
    goto fail; //Enum
    }
