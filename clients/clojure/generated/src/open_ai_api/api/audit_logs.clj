...
(defn-spec list-audit-logs-with-http-info any?
  "List user actions and configuration changes within this organization."
  ([] (list-audit-logs-with-http-info nil))
  ([{:keys [effective_at project_ids event_types actor_ids actor_emails resource_ids limit after before]} (s/map-of keyword? any?)]
   (call-api "/organization/audit_logs" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"effective_at" effective_at "project_ids[]" (with-collection-format project_ids :multi) "event_types[]" (with-collection-format event_types :multi) "actor_ids[]" (with-collection-format actor_ids :multi) "actor_emails[]" (with-collection-format actor_emails :multi) "resource_ids[]" (with-collection-format resource_ids :multi) "limit" limit "after" after "before" before }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec list-audit-logs list-audit-logs-response-spec
  "List user actions and configuration changes within this organization."
  ([] (list-audit-logs nil))
  ([optional-params any?]
   (let [res (:data (list-audit-logs-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode list-audit-logs-response-spec res st/string-transformer)
        res))))
