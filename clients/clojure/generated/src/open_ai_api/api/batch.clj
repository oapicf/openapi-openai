...
(defn-spec cancel-batch-with-http-info any?
  "Cancels an in-progress batch. The batch will be in status `cancelling` for up to 10 minutes, before changing to `cancelled`, where it will have partial results (if any) available in the output file."
  [batch_id string?]
  (check-required-params batch_id)
  (call-api "/batches/{batch_id}/cancel" :post
...
(defn-spec cancel-batch batch-spec
  "Cancels an in-progress batch. The batch will be in status `cancelling` for up to 10 minutes, before changing to `cancelled`, where it will have partial results (if any) available in the output file."
  [batch_id string?]
  (let [res (:data (cancel-batch-with-http-info batch_id))]
    (if (:decode-models *api-context*)
       (st/decode batch-spec res st/string-transformer)
       res)))
...
(defn-spec create-batch-with-http-info any?
  ...
  (check-required-params create-batch-request)
  (call-api "/batches" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    create-batch-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec create-batch batch-spec
  "Creates and executes a batch from an uploaded file of requests"
  [create-batch-request create-batch-request]
  (let [res (:data (create-batch-with-http-info create-batch-request))]
    (if (:decode-models *api-context*)
       (st/decode batch-spec res st/string-transformer)
       res)))


(defn-spec list-batches-with-http-info any?
  "List your organization's batches."
  ([] (list-batches-with-http-info nil))
  ([{:keys [after limit]} (s/map-of keyword? any?)]
   (call-api "/batches" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec list-batches list-batches-response-spec
  "List your organization's batches."
  ([] (list-batches nil))
  ([optional-params any?]
   (let [res (:data (list-batches-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode list-batches-response-spec res st/string-transformer)
        res))))


(defn-spec retrieve-batch-with-http-info any?
  "Retrieves a batch."
  [batch_id string?]
  (check-required-params batch_id)
  (call-api "/batches/{batch_id}" :get
            {:path-params   {"batch_id" batch_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-batch batch-spec
  "Retrieves a batch."
  [batch_id string?]
  (let [res (:data (retrieve-batch-with-http-info batch_id))]
    (if (:decode-models *api-context*)
       (st/decode batch-spec res st/string-transformer)
       res)))
