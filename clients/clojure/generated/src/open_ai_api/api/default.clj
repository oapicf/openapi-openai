...
(defn-spec admin-api-keys-create-with-http-info any?
  "Create an organization admin API key
  Create a new admin-level API key for the organization."
  [admin-api-keys-create-request admin-api-keys-create-request]
  (check-required-params admin-api-keys-create-request)
  (call-api "/organization/admin_api_keys" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    admin-api-keys-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec admin-api-keys-create admin-api-key-spec
  "Create an organization admin API key
  Create a new admin-level API key for the organization."
  [admin-api-keys-create-request admin-api-keys-create-request]
  (let [res (:data (admin-api-keys-create-with-http-info admin-api-keys-create-request))]
    (if (:decode-models *api-context*)
       (st/decode admin-api-key-spec res st/string-transformer)
       res)))


(defn-spec admin-api-keys-delete-with-http-info any?
  "Delete an organization admin API key
  Delete the specified admin API key."
  [key_id string?]
  (check-required-params key_id)
  (call-api "/organization/admin_api_keys/{key_id}" :delete
            {:path-params   {"key_id" key_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec admin-api-keys-delete admin-api-keys-delete-200-response-spec
  "Delete an organization admin API key
  Delete the specified admin API key."
  [key_id string?]
  (let [res (:data (admin-api-keys-delete-with-http-info key_id))]
    (if (:decode-models *api-context*)
       (st/decode admin-api-keys-delete-200-response-spec res st/string-transformer)
       res)))


(defn-spec admin-api-keys-get-with-http-info any?
  "Retrieve a single organization API key
  Get details for a specific organization API key by its ID."
  [key_id string?]
  (check-required-params key_id)
  (call-api "/organization/admin_api_keys/{key_id}" :get
            {:path-params   {"key_id" key_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec admin-api-keys-get admin-api-key-spec
  "Retrieve a single organization API key
  Get details for a specific organization API key by its ID."
  [key_id string?]
  (let [res (:data (admin-api-keys-get-with-http-info key_id))]
    (if (:decode-models *api-context*)
       (st/decode admin-api-key-spec res st/string-transformer)
       res)))


(defn-spec admin-api-keys-list-with-http-info any?
  "List organization API keys
  Retrieve a paginated list of organization admin API keys."
  ([] (admin-api-keys-list-with-http-info nil))
  ([{:keys [after order limit]} (s/map-of keyword? any?)]
   (call-api "/organization/admin_api_keys" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"after" after "order" order "limit" limit }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec admin-api-keys-list api-key-list-spec
  "List organization API keys
  Retrieve a paginated list of organization admin API keys."
  ([] (admin-api-keys-list nil))
  ([optional-params any?]
   (let [res (:data (admin-api-keys-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode api-key-list-spec res st/string-transformer)
        res))))
