...
(defn-spec create-image-with-http-info any?
  "Creates an image given a prompt."
  [create-image-request create-image-request]
  (check-required-params create-image-request)
  (call-api "/images/generations" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    create-image-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec create-image images-response-spec
  "Creates an image given a prompt."
  [create-image-request create-image-request]
  (let [res (:data (create-image-with-http-info create-image-request))]
    (if (:decode-models *api-context*)
       (st/decode images-response-spec res st/string-transformer)
       res)))


(defn-spec create-image-edit-with-http-info any?
  "Creates an edited or extended image given an original image and a prompt."
  ([^File image any?, prompt string?, ] (create-image-edit-with-http-info image prompt nil))
  ([^File image any?, prompt string?, {:keys [^File mask model n size response_format user]} (s/map-of keyword? any?)]
   (check-required-params image prompt)
   (call-api "/images/edits" :post
             {:path-params   {}
              :header-params {}
              :query-params  {}
              :form-params   {"image" image "prompt" prompt "mask" mask "model" model "n" n "size" size "response_format" response_format "user" user }
              :content-types ["multipart/form-data"]
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec create-image-edit images-response-spec
  "Creates an edited or extended image given an original image and a prompt."
  ([^File image any?, prompt string?, ] (create-image-edit image prompt nil))
  ([^File image any?, prompt string?, optional-params any?]
   (let [res (:data (create-image-edit-with-http-info image prompt optional-params))]
     (if (:decode-models *api-context*)
        (st/decode images-response-spec res st/string-transformer)
        res))))


(defn-spec create-image-variation-with-http-info any?
  "Creates a variation of a given image."
  ([^File image any?, ] (create-image-variation-with-http-info image nil))
  ([^File image any?, {:keys [model n response_format size user]} (s/map-of keyword? any?)]
   (check-required-params image)
   (call-api "/images/variations" :post
             {:path-params   {}
              :header-params {}
              :query-params  {}
              :form-params   {"image" image "model" model "n" n "response_format" response_format "size" size "user" user }
              :content-types ["multipart/form-data"]
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec create-image-variation images-response-spec
  "Creates a variation of a given image."
  ([^File image any?, ] (create-image-variation image nil))
  ([^File image any?, optional-params any?]
   (let [res (:data (create-image-variation-with-http-info image optional-params))]
     (if (:decode-models *api-context*)
        (st/decode images-response-spec res st/string-transformer)
        res))))
