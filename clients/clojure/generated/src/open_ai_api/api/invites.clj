...
(defn-spec delete-invite-with-http-info any?
  "Delete an invite. If the invite has already been accepted, it cannot be deleted."
  [invite_id string?]
  (check-required-params invite_id)
  (call-api "/organization/invites/{invite_id}" :delete
            {:path-params   {"invite_id" invite_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec delete-invite invite-delete-response-spec
  "Delete an invite. If the invite has already been accepted, it cannot be deleted."
  [invite_id string?]
  (let [res (:data (delete-invite-with-http-info invite_id))]
    (if (:decode-models *api-context*)
       (st/decode invite-delete-response-spec res st/string-transformer)
       res)))
...
(defn-spec invite-user-with-http-info any?
  ...
  (call-api "/organization/invites" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec invite-user invite-spec
  "Create an invite for a user to the organization. The invite must be accepted by the user before they have access to the organization."
  [invite-request invite-request]
  (let [res (:data (invite-user-with-http-info invite-request))]
    (if (:decode-models *api-context*)
       (st/decode invite-spec res st/string-transformer)
       res)))


(defn-spec list-invites-with-http-info any?
  "Returns a list of invites in the organization."
  ([] (list-invites-with-http-info nil))
  ([{:keys [limit after]} (s/map-of keyword? any?)]
   (call-api "/organization/invites" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec list-invites invite-list-response-spec
  "Returns a list of invites in the organization."
  ([] (list-invites nil))
  ([optional-params any?]
   (let [res (:data (list-invites-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode invite-list-response-spec res st/string-transformer)
        res))))


(defn-spec retrieve-invite-with-http-info any?
  "Retrieves an invite."
  [invite_id string?]
  (check-required-params invite_id)
  (call-api "/organization/invites/{invite_id}" :get
            {:path-params   {"invite_id" invite_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-invite invite-spec
  "Retrieves an invite."
  [invite_id string?]
  (let [res (:data (retrieve-invite-with-http-info invite_id))]
    (if (:decode-models *api-context*)
       (st/decode invite-spec res st/string-transformer)
       res)))
