...
(defn-spec archive-project-with-http-info any?
  "Archives a project in the organization. Archived projects cannot be used or updated."
  [project_id string?]
  (check-required-params project_id)
  (call-api "/organization/projects/{project_id}/archive" :post
...
(defn-spec archive-project project-spec
  "Archives a project in the organization. Archived projects cannot be used or updated."
  [project_id string?]
  (let [res (:data (archive-project-with-http-info project_id))]
    (if (:decode-models *api-context*)
       (st/decode project-spec res st/string-transformer)
       res)))


(defn-spec create-project-with-http-info any?
  "Create a new project in the organization. Projects can be created and archived, but cannot be deleted."
  [project-create-request project-create-request]
  (check-required-params project-create-request)
  (call-api "/organization/projects" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    project-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec create-project project-spec
  "Create a new project in the organization. Projects can be created and archived, but cannot be deleted."
  [project-create-request project-create-request]
  (let [res (:data (create-project-with-http-info project-create-request))]
    (if (:decode-models *api-context*)
       (st/decode project-spec res st/string-transformer)
       res)))


(defn-spec create-project-service-account-with-http-info any?
  "Creates a new service account in the project. This also returns an unredacted API key for the service account."
  [project_id string?, project-service-account-create-request project-service-account-create-request]
  (check-required-params project_id project-service-account-create-request)
  (call-api "/organization/projects/{project_id}/service_accounts" :post
            {:path-params   {"project_id" project_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    project-service-account-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec create-project-service-account project-service-account-create-response-spec
  "Creates a new service account in the project. This also returns an unredacted API key for the service account."
  [project_id string?, project-service-account-create-request project-service-account-create-request]
  (let [res (:data (create-project-service-account-with-http-info project_id project-service-account-create-request))]
    (if (:decode-models *api-context*)
       (st/decode project-service-account-create-response-spec res st/string-transformer)
       res)))


(defn-spec create-project-user-with-http-info any?
  "Adds a user to the project. Users must already be members of the organization to be added to a project."
  [project_id string?, project-user-create-request project-user-create-request]
  (check-required-params project_id project-user-create-request)
  (call-api "/organization/projects/{project_id}/users" :post
            {:path-params   {"project_id" project_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    project-user-create-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec create-project-user project-user-spec
  "Adds a user to the project. Users must already be members of the organization to be added to a project."
  [project_id string?, project-user-create-request project-user-create-request]
  (let [res (:data (create-project-user-with-http-info project_id project-user-create-request))]
    (if (:decode-models *api-context*)
       (st/decode project-user-spec res st/string-transformer)
       res)))


(defn-spec delete-project-api-key-with-http-info any?
  "Deletes an API key from the project."
  [project_id string?, key_id string?]
  (check-required-params project_id key_id)
  (call-api "/organization/projects/{project_id}/api_keys/{key_id}" :delete
            {:path-params   {"project_id" project_id "key_id" key_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec delete-project-api-key project-api-key-delete-response-spec
  "Deletes an API key from the project."
  [project_id string?, key_id string?]
  (let [res (:data (delete-project-api-key-with-http-info project_id key_id))]
    (if (:decode-models *api-context*)
       (st/decode project-api-key-delete-response-spec res st/string-transformer)
       res)))


(defn-spec delete-project-service-account-with-http-info any?
  "Deletes a service account from the project."
  [project_id string?, service_account_id string?]
  (check-required-params project_id service_account_id)
  (call-api "/organization/projects/{project_id}/service_accounts/{service_account_id}" :delete
            {:path-params   {"project_id" project_id "service_account_id" service_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :content-types []
...
(defn-spec delete-project-service-account project-service-account-delete-response-spec
  "Deletes a service account from the project."
  [project_id string?, service_account_id string?]
  (let [res (:data (delete-project-service-account-with-http-info project_id service_account_id))]
    (if (:decode-models *api-context*)
       (st/decode project-service-account-delete-response-spec res st/string-transformer)
       res)))


(defn-spec delete-project-user-with-http-info any?
  "Deletes a user from the project."
  [project_id string?, user_id string?]
  (check-required-params project_id user_id)
  (call-api "/organization/projects/{project_id}/users/{user_id}" :delete
...
(defn-spec delete-project-user project-user-delete-response-spec
  "Deletes a user from the project."
  [project_id string?, user_id string?]
  (let [res (:data (delete-project-user-with-http-info project_id user_id))]
    (if (:decode-models *api-context*)
       (st/decode project-user-delete-response-spec res st/string-transformer)
       res)))


(defn-spec list-project-api-keys-with-http-info any?
  "Returns a list of API keys in the project."
  ([project_id string?, ] (list-project-api-keys-with-http-info project_id nil))
  ([project_id string?, {:keys [limit after]} (s/map-of keyword? any?)]
   (check-required-params project_id)
   (call-api "/organization/projects/{project_id}/api_keys" :get
             {:path-params   {"project_id" project_id }
              :header-params {}
              :query-params  {"limit" limit "after" after }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec list-project-api-keys project-api-key-list-response-spec
  "Returns a list of API keys in the project."
  ([project_id string?, ] (list-project-api-keys project_id nil))
  ([project_id string?, optional-params any?]
   (let [res (:data (list-project-api-keys-with-http-info project_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode project-api-key-list-response-spec res st/string-transformer)
        res))))


(defn-spec list-project-rate-limits-with-http-info any?
  "Returns the rate limits per model for a project."
  ([project_id string?, ] (list-project-rate-limits-with-http-info project_id nil))
  ([project_id string?, {:keys [limit after before]} (s/map-of keyword? any?)]
   (check-required-params project_id)
   (call-api "/organization/projects/{project_id}/rate_limits" :get
             {:path-params   {"project_id" project_id }
              :header-params {}
              :query-params  {"limit" limit "after" after "before" before }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec list-project-rate-limits project-rate-limit-list-response-spec
  "Returns the rate limits per model for a project."
  ([project_id string?, ] (list-project-rate-limits project_id nil))
  ([project_id string?, optional-params any?]
   (let [res (:data (list-project-rate-limits-with-http-info project_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode project-rate-limit-list-response-spec res st/string-transformer)
        res))))


(defn-spec list-project-service-accounts-with-http-info any?
  "Returns a list of service accounts in the project."
  ([project_id string?, ] (list-project-service-accounts-with-http-info project_id nil))
  ([project_id string?, {:keys [limit after]} (s/map-of keyword? any?)]
   (check-required-params project_id)
   (call-api "/organization/projects/{project_id}/service_accounts" :get
             {:path-params   {"project_id" project_id }
              :header-params {}
              :query-params  {"limit" limit "after" after }
              :form-params   {}
...
(defn-spec list-project-service-accounts project-service-account-list-response-spec
  "Returns a list of service accounts in the project."
  ([project_id string?, ] (list-project-service-accounts project_id nil))
  ([project_id string?, optional-params any?]
   (let [res (:data (list-project-service-accounts-with-http-info project_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode project-service-account-list-response-spec res st/string-transformer)
        res))))


(defn-spec list-project-users-with-http-info any?
  "Returns a list of users in the project."
  ([project_id string?, ] (list-project-users-with-http-info project_id nil))
  ([project_id string?, {:keys [limit after]} (s/map-of keyword? any?)]
   (check-required-params project_id)
   (call-api "/organization/projects/{project_id}/users" :get
             {:path-params   {"project_id" project_id }
              :header-params {}
              :query-params  {"limit" limit "after" after }
              :form-params   {}
...
(defn-spec list-project-users project-user-list-response-spec
  "Returns a list of users in the project."
  ([project_id string?, ] (list-project-users project_id nil))
  ([project_id string?, optional-params any?]
   (let [res (:data (list-project-users-with-http-info project_id optional-params))]
     (if (:decode-models *api-context*)
        (st/decode project-user-list-response-spec res st/string-transformer)
        res))))


(defn-spec list-projects-with-http-info any?
  "Returns a list of projects."
  ([] (list-projects-with-http-info nil))
  ([{:keys [limit after include_archived]} (s/map-of keyword? any?)]
   (call-api "/organization/projects" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"limit" limit "after" after "include_archived" include_archived }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec list-projects project-list-response-spec
  "Returns a list of projects."
  ([] (list-projects nil))
  ([optional-params any?]
   (let [res (:data (list-projects-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode project-list-response-spec res st/string-transformer)
        res))))


(defn-spec modify-project-with-http-info any?
  "Modifies a project in the organization."
  [project_id string?, project-update-request project-update-request]
  (check-required-params project_id project-update-request)
  (call-api "/organization/projects/{project_id}" :post
            {:path-params   {"project_id" project_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    project-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec modify-project project-spec
  "Modifies a project in the organization."
  [project_id string?, project-update-request project-update-request]
  (let [res (:data (modify-project-with-http-info project_id project-update-request))]
    (if (:decode-models *api-context*)
       (st/decode project-spec res st/string-transformer)
       res)))


(defn-spec modify-project-user-with-http-info any?
  "Modifies a user's role in the project."
  [project_id string?, user_id string?, project-user-update-request project-user-update-request]
  (check-required-params project_id user_id project-user-update-request)
  (call-api "/organization/projects/{project_id}/users/{user_id}" :post
            {:path-params   {"project_id" project_id "user_id" user_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    project-user-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec modify-project-user project-user-spec
  "Modifies a user's role in the project."
  [project_id string?, user_id string?, project-user-update-request project-user-update-request]
  (let [res (:data (modify-project-user-with-http-info project_id user_id project-user-update-request))]
    (if (:decode-models *api-context*)
       (st/decode project-user-spec res st/string-transformer)
       res)))


(defn-spec retrieve-project-with-http-info any?
  "Retrieves a project."
  [project_id string?]
  (check-required-params project_id)
  (call-api "/organization/projects/{project_id}" :get
            {:path-params   {"project_id" project_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-project project-spec
  "Retrieves a project."
  [project_id string?]
  (let [res (:data (retrieve-project-with-http-info project_id))]
    (if (:decode-models *api-context*)
       (st/decode project-spec res st/string-transformer)
       res)))


(defn-spec retrieve-project-api-key-with-http-info any?
  "Retrieves an API key in the project."
  [project_id string?, key_id string?]
  (check-required-params project_id key_id)
  (call-api "/organization/projects/{project_id}/api_keys/{key_id}" :get
            {:path-params   {"project_id" project_id "key_id" key_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-project-api-key project-api-key-spec
  "Retrieves an API key in the project."
  [project_id string?, key_id string?]
  (let [res (:data (retrieve-project-api-key-with-http-info project_id key_id))]
    (if (:decode-models *api-context*)
       (st/decode project-api-key-spec res st/string-transformer)
       res)))


(defn-spec retrieve-project-service-account-with-http-info any?
  "Retrieves a service account in the project."
  [project_id string?, service_account_id string?]
  (check-required-params project_id service_account_id)
  (call-api "/organization/projects/{project_id}/service_accounts/{service_account_id}" :get
            {:path-params   {"project_id" project_id "service_account_id" service_account_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-project-service-account project-service-account-spec
  "Retrieves a service account in the project."
  [project_id string?, service_account_id string?]
  (let [res (:data (retrieve-project-service-account-with-http-info project_id service_account_id))]
    (if (:decode-models *api-context*)
       (st/decode project-service-account-spec res st/string-transformer)
       res)))


(defn-spec retrieve-project-user-with-http-info any?
  "Retrieves a user in the project."
  [project_id string?, user_id string?]
  (check-required-params project_id user_id)
  (call-api "/organization/projects/{project_id}/users/{user_id}" :get
            {:path-params   {"project_id" project_id "user_id" user_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-project-user project-user-spec
  "Retrieves a user in the project."
  [project_id string?, user_id string?]
  (let [res (:data (retrieve-project-user-with-http-info project_id user_id))]
    (if (:decode-models *api-context*)
       (st/decode project-user-spec res st/string-transformer)
       res)))


(defn-spec update-project-rate-limits-with-http-info any?
  "Updates a project rate limit."
  [project_id string?, rate_limit_id string?, project-rate-limit-update-request project-rate-limit-update-request]
  (check-required-params project_id rate_limit_id project-rate-limit-update-request)
  (call-api "/organization/projects/{project_id}/rate_limits/{rate_limit_id}" :post
            {:path-params   {"project_id" project_id "rate_limit_id" rate_limit_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    project-rate-limit-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec update-project-rate-limits project-rate-limit-spec
  "Updates a project rate limit."
  [project_id string?, rate_limit_id string?, project-rate-limit-update-request project-rate-limit-update-request]
  (let [res (:data (update-project-rate-limits-with-http-info project_id rate_limit_id project-rate-limit-update-request))]
    (if (:decode-models *api-context*)
       (st/decode project-rate-limit-spec res st/string-transformer)
       res)))
