...
(defn-spec add-upload-part-with-http-info any?
  "Adds a [Part](/docs/api-reference/uploads/part-object) to an [Upload](/docs/api-reference/uploads/object) object. A Part represents a chunk of bytes from the file you are trying to upload. 

Each Part can be at most 64 MB, and you can add Parts until you hit the Upload maximum of 8 GB.

It is possible to add multiple Parts in parallel. You can decide the intended order of the Parts when you [complete the Upload](/docs/api-reference/uploads/complete)."
  [upload_id string?, ^File data any?]
  (check-required-params upload_id data)
  (call-api "/uploads/{upload_id}/parts" :post
            {:path-params   {"upload_id" upload_id }
             :header-params {}
             :query-params  {}
...
(defn-spec add-upload-part upload-part-spec
  "Adds a [Part](/docs/api-reference/uploads/part-object) to an [Upload](/docs/api-reference/uploads/object) object. A Part represents a chunk of bytes from the file you are trying to upload. 
...
It is possible to add multiple Parts in parallel. You can decide the intended order of the Parts when you [complete the Upload](/docs/api-reference/uploads/complete)."
  [upload_id string?, ^File data any?]
  (let [res (:data (add-upload-part-with-http-info upload_id data))]
    (if (:decode-models *api-context*)
       (st/decode upload-part-spec res st/string-transformer)
       res)))


(defn-spec cancel-upload-with-http-info any?
  "Cancels the Upload. No Parts may be added after an Upload is cancelled."
  [upload_id string?]
  (check-required-params upload_id)
  (call-api "/uploads/{upload_id}/cancel" :post
            {:path-params   {"upload_id" upload_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec cancel-upload upload-spec
  "Cancels the Upload. No Parts may be added after an Upload is cancelled."
  [upload_id string?]
  (let [res (:data (cancel-upload-with-http-info upload_id))]
    (if (:decode-models *api-context*)
       (st/decode upload-spec res st/string-transformer)
       res)))


(defn-spec complete-upload-with-http-info any?
...
The number of bytes uploaded upon completion must match the number of bytes initially specified when creating the Upload object. No Parts may be added after an Upload is completed."
  [upload_id string?, complete-upload-request complete-upload-request]
  (check-required-params upload_id complete-upload-request)
  (call-api "/uploads/{upload_id}/complete" :post
            {:path-params   {"upload_id" upload_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    complete-upload-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec complete-upload upload-spec
  "Completes the [Upload](/docs/api-reference/uploads/object). 
...
The number of bytes uploaded upon completion must match the number of bytes initially specified when creating the Upload object. No Parts may be added after an Upload is completed."
  [upload_id string?, complete-upload-request complete-upload-request]
  (let [res (:data (complete-upload-with-http-info upload_id complete-upload-request))]
    (if (:decode-models *api-context*)
       (st/decode upload-spec res st/string-transformer)
       res)))


(defn-spec create-upload-with-http-info any?
...
For guidance on the proper filename extensions for each purpose, please follow the documentation on [creating a File](/docs/api-reference/files/create)."
  ...
  (check-required-params create-upload-request)
  (call-api "/uploads" :post
            {:path-params   {}
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    create-upload-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec create-upload upload-spec
  "Creates an intermediate [Upload](/docs/api-reference/uploads/object) object that you can add [Parts](/docs/api-reference/uploads/part-object) to. Currently, an Upload can accept at most 8 GB in total and expires after an hour after you create it.
...
For guidance on the proper filename extensions for each purpose, please follow the documentation on [creating a File](/docs/api-reference/files/create)."
  [create-upload-request create-upload-request]
  (let [res (:data (create-upload-with-http-info create-upload-request))]
    (if (:decode-models *api-context*)
       (st/decode upload-spec res st/string-transformer)
       res)))
