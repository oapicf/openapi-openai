...
(defn-spec usage-audio-speeches-with-http-info any?
  "Get audio speeches usage details for the organization."
  ([start_time int?, ] (usage-audio-speeches-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids user_ids api_key_ids models group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/audio_speeches" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "user_ids" (with-collection-format user_ids :multi) "api_key_ids" (with-collection-format api_key_ids :multi) "models" (with-collection-format models :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec usage-audio-speeches usage-response-spec
  "Get audio speeches usage details for the organization."
  ([start_time int?, ] (usage-audio-speeches start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-audio-speeches-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-audio-transcriptions-with-http-info any?
  "Get audio transcriptions usage details for the organization."
  ([start_time int?, ] (usage-audio-transcriptions-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids user_ids api_key_ids models group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/audio_transcriptions" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "user_ids" (with-collection-format user_ids :multi) "api_key_ids" (with-collection-format api_key_ids :multi) "models" (with-collection-format models :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
...
(defn-spec usage-audio-transcriptions usage-response-spec
  "Get audio transcriptions usage details for the organization."
  ([start_time int?, ] (usage-audio-transcriptions start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-audio-transcriptions-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-code-interpreter-sessions-with-http-info any?
  "Get code interpreter sessions usage details for the organization."
  ([start_time int?, ] (usage-code-interpreter-sessions-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/code_interpreter_sessions" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec usage-code-interpreter-sessions usage-response-spec
  "Get code interpreter sessions usage details for the organization."
  ([start_time int?, ] (usage-code-interpreter-sessions start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-code-interpreter-sessions-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-completions-with-http-info any?
  "Get completions usage details for the organization."
  ([start_time int?, ] (usage-completions-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids user_ids api_key_ids models batch group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/completions" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "user_ids" (with-collection-format user_ids :multi) "api_key_ids" (with-collection-format api_key_ids :multi) "models" (with-collection-format models :multi) "batch" batch "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec usage-completions usage-response-spec
  "Get completions usage details for the organization."
  ([start_time int?, ] (usage-completions start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-completions-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-costs-with-http-info any?
  "Get costs details for the organization."
  ([start_time int?, ] (usage-costs-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/costs" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
...
(defn-spec usage-costs usage-response-spec
  "Get costs details for the organization."
  ([start_time int?, ] (usage-costs start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-costs-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-embeddings-with-http-info any?
  "Get embeddings usage details for the organization."
  ([start_time int?, ] (usage-embeddings-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids user_ids api_key_ids models group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/embeddings" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "user_ids" (with-collection-format user_ids :multi) "api_key_ids" (with-collection-format api_key_ids :multi) "models" (with-collection-format models :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
...
(defn-spec usage-embeddings usage-response-spec
  "Get embeddings usage details for the organization."
  ([start_time int?, ] (usage-embeddings start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-embeddings-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-images-with-http-info any?
  "Get images usage details for the organization."
  ([start_time int?, ] (usage-images-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width sources sizes project_ids user_ids api_key_ids models group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/images" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "sources" (with-collection-format sources :multi) "sizes" (with-collection-format sizes :multi) "project_ids" (with-collection-format project_ids :multi) "user_ids" (with-collection-format user_ids :multi) "api_key_ids" (with-collection-format api_key_ids :multi) "models" (with-collection-format models :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
              :content-types []
              :accepts       ["application/json"]
              :auth-names    ["ApiKeyAuth"]})))

(defn-spec usage-images usage-response-spec
  "Get images usage details for the organization."
  ([start_time int?, ] (usage-images start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-images-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-moderations-with-http-info any?
  "Get moderations usage details for the organization."
  ([start_time int?, ] (usage-moderations-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids user_ids api_key_ids models group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/moderations" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "user_ids" (with-collection-format user_ids :multi) "api_key_ids" (with-collection-format api_key_ids :multi) "models" (with-collection-format models :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
...
(defn-spec usage-moderations usage-response-spec
  "Get moderations usage details for the organization."
  ([start_time int?, ] (usage-moderations start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-moderations-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))


(defn-spec usage-vector-stores-with-http-info any?
  "Get vector stores usage details for the organization."
  ([start_time int?, ] (usage-vector-stores-with-http-info start_time nil))
  ([start_time int?, {:keys [end_time bucket_width project_ids group_by limit page]} (s/map-of keyword? any?)]
   (check-required-params start_time)
   (call-api "/organization/usage/vector_stores" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"start_time" start_time "end_time" end_time "bucket_width" bucket_width "project_ids" (with-collection-format project_ids :multi) "group_by" (with-collection-format group_by :multi) "limit" limit "page" page }
              :form-params   {}
...
(defn-spec usage-vector-stores usage-response-spec
  "Get vector stores usage details for the organization."
  ([start_time int?, ] (usage-vector-stores start_time nil))
  ([start_time int?, optional-params any?]
   (let [res (:data (usage-vector-stores-with-http-info start_time optional-params))]
     (if (:decode-models *api-context*)
        (st/decode usage-response-spec res st/string-transformer)
        res))))
