...
(defn-spec delete-user-with-http-info any?
  ...
  (call-api "/organization/users/{user_id}" :delete
...
(defn-spec delete-user user-delete-response-spec
  "Deletes a user from the organization."
  [user_id string?]
  (let [res (:data (delete-user-with-http-info user_id))]
    (if (:decode-models *api-context*)
       (st/decode user-delete-response-spec res st/string-transformer)
       res)))
...
(defn-spec list-users-with-http-info any?
  ...
  ([{:keys [limit after]} (s/map-of keyword? any?)]
   (call-api "/organization/users" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec list-users user-list-response-spec
  "Lists all of the users in the organization."
  ([] (list-users nil))
  ([optional-params any?]
   (let [res (:data (list-users-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode user-list-response-spec res st/string-transformer)
        res))))


(defn-spec modify-user-with-http-info any?
  "Modifies a user's role in the organization."
  [user_id string?, user-role-update-request user-role-update-request]
  (check-required-params user_id user-role-update-request)
  (call-api "/organization/users/{user_id}" :post
            {:path-params   {"user_id" user_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
             :body-param    user-role-update-request
             :content-types ["application/json"]
             :accepts       ["application/json"]
             :auth-names    ["ApiKeyAuth"]}))

(defn-spec modify-user user-spec
  "Modifies a user's role in the organization."
  [user_id string?, user-role-update-request user-role-update-request]
  (let [res (:data (modify-user-with-http-info user_id user-role-update-request))]
    (if (:decode-models *api-context*)
       (st/decode user-spec res st/string-transformer)
       res)))


(defn-spec retrieve-user-with-http-info any?
  "Retrieves a user by their identifier."
  [user_id string?]
  (check-required-params user_id)
  (call-api "/organization/users/{user_id}" :get
            {:path-params   {"user_id" user_id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec retrieve-user user-spec
  "Retrieves a user by their identifier."
  [user_id string?]
  (let [res (:data (retrieve-user-with-http-info user_id))]
    (if (:decode-models *api-context*)
       (st/decode user-spec res st/string-transformer)
       res)))
