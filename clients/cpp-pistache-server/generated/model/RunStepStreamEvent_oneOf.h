/**
* OpenAI API
* The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
*
* The version of the OpenAPI document: 2.0.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * RunStepStreamEvent_oneOf.h
 *
 * Occurs when a [run step](/docs/api-reference/runs/step-object) is created.
 */

#ifndef RunStepStreamEvent_oneOf_H_
#define RunStepStreamEvent_oneOf_H_


#include "RunStepObject.h"
#include <string>
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// Occurs when a [run step](/docs/api-reference/runs/step-object) is created.
/// </summary>
class  RunStepStreamEvent_oneOf
{
public:
    RunStepStreamEvent_oneOf();
    virtual ~RunStepStreamEvent_oneOf() = default;


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const RunStepStreamEvent_oneOf& rhs) const;
    bool operator!=(const RunStepStreamEvent_oneOf& rhs) const;

    /////////////////////////////////////////////
    /// RunStepStreamEvent_oneOf members

    /// <summary>
    /// 
    /// </summary>
    std::string getEvent() const;
    void setEvent(std::string const& value);
    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::RunStepObject getData() const;
    void setData(org::openapitools::server::model::RunStepObject const& value);

    friend  void to_json(nlohmann::json& j, const RunStepStreamEvent_oneOf& o);
    friend  void from_json(const nlohmann::json& j, RunStepStreamEvent_oneOf& o);
protected:
    std::string m_Event;

    org::openapitools::server::model::RunStepObject m_Data;

    
};

} // namespace org::openapitools::server::model

#endif /* RunStepStreamEvent_oneOf_H_ */
