/**
* OpenAI API
* The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
*
* The version of the OpenAPI document: 2.0.0
* Contact: blah+oapicf@cliffano.com
*
* NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
* https://openapi-generator.tech
* Do not edit the class manually.
*/
/*
 * RunStepStreamEvent_oneOf_4.h
 *
 * Occurs when a [run step](/docs/api-reference/runs/step-object) fails.
 */

#ifndef RunStepStreamEvent_oneOf_4_H_
#define RunStepStreamEvent_oneOf_4_H_


#include "RunStepObject.h"
#include <string>
#include <nlohmann/json.hpp>

namespace org::openapitools::server::model
{

/// <summary>
/// Occurs when a [run step](/docs/api-reference/runs/step-object) fails.
/// </summary>
class  RunStepStreamEvent_oneOf_4
{
public:
    RunStepStreamEvent_oneOf_4();
    virtual ~RunStepStreamEvent_oneOf_4() = default;


    /// <summary>
    /// Validate the current data in the model. Throws a ValidationException on failure.
    /// </summary>
    void validate() const;

    /// <summary>
    /// Validate the current data in the model. Returns false on error and writes an error
    /// message into the given stringstream.
    /// </summary>
    bool validate(std::stringstream& msg) const;

    /// <summary>
    /// Helper overload for validate. Used when one model stores another model and calls it's validate.
    /// Not meant to be called outside that case.
    /// </summary>
    bool validate(std::stringstream& msg, const std::string& pathPrefix) const;

    bool operator==(const RunStepStreamEvent_oneOf_4& rhs) const;
    bool operator!=(const RunStepStreamEvent_oneOf_4& rhs) const;

    /////////////////////////////////////////////
    /// RunStepStreamEvent_oneOf_4 members

    /// <summary>
    /// 
    /// </summary>
    std::string getEvent() const;
    void setEvent(std::string const& value);
    /// <summary>
    /// 
    /// </summary>
    org::openapitools::server::model::RunStepObject getData() const;
    void setData(org::openapitools::server::model::RunStepObject const& value);

    friend  void to_json(nlohmann::json& j, const RunStepStreamEvent_oneOf_4& o);
    friend  void from_json(const nlohmann::json& j, RunStepStreamEvent_oneOf_4& o);
protected:
    std::string m_Event;

    org::openapitools::server::model::RunStepObject m_Data;

    
};

} // namespace org::openapitools::server::model

#endif /* RunStepStreamEvent_oneOf_4_H_ */
