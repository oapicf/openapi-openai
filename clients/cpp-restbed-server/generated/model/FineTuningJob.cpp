...
ptree FineTuningJob::toPropertyTree() const
{
	...
	pt.put("seed", m_Seed);
	pt.put("estimated_finish", m_Estimated_finish);
...
void FineTuningJob::fromPropertyTree(ptree const &pt)
{
	...
	m_Estimated_finish = pt.get("estimated_finish", 0);
	if (pt.get_child_optional("method")) {
        m_Method = fromPt<FineTuneMethod>(pt.get_child("method"));
	}
}
...
FineTuneMethod FineTuningJob::getMethod() const
{
    return m_Method;
}

void FineTuningJob::setMethod(FineTuneMethod value)
{
    m_Method = value;
}
