...
CreateCompletionResponse_choices_inner_logprobs::toJson()
{
	...
	if (isprimitive("std::string")) {
		list<std::string> new_list = static_cast<list <std::string> > (getTopLogprobs());
		node = converttoJson(&new_list, "std::string", "array");
	} else {
		node = json_node_alloc();
		list<std::string> new_list = static_cast<list <std::string> > (getTopLogprobs());
...
void
CreateCompletionResponse_choices_inner_logprobs::setTopLogprobs(std::list <std::string> top_logprobs)
{
