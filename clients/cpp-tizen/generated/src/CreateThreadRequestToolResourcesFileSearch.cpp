...
void
CreateThreadRequest_tool_resources_file_search::__init()
{
...
void
CreateThreadRequest_tool_resources_file_search::__cleanup()
{
...
void
CreateThreadRequest_tool_resources_file_search::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			JsonArray* arr = json_node_get_array(node);
			JsonNode*  temp_json;
			list<CreateThreadRequest_tool_resources_file_search_vector_stores_inner> new_list;
			CreateThreadRequest_tool_resources_file_search_vector_stores_inner inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("CreateThreadRequest_tool_resources_file_search_vector_stores_inner")) {
					jsonToValue(&inst, temp_json, "CreateThreadRequest_tool_resources_file_search_vector_stores_inner", "");
				} else {
...
CreateThreadRequest_tool_resources_file_search::CreateThreadRequest_tool_resources_file_search(char* json)
{
	this->fromJson(json);
}
...
CreateThreadRequest_tool_resources_file_search::toJson()
{
	...
	if (isprimitive("CreateThreadRequest_tool_resources_file_search_vector_stores_inner")) {
		list<CreateThreadRequest_tool_resources_file_search_vector_stores_inner> new_list = static_cast<list <CreateThreadRequest_tool_resources_file_search_vector_stores_inner> > (getVectorStores());
		node = converttoJson(&new_list, "CreateThreadRequest_tool_resources_file_search_vector_stores_inner", "array");
	} else {
		node = json_node_alloc();
		list<CreateThreadRequest_tool_resources_file_search_vector_stores_inner> new_list = static_cast<list <CreateThreadRequest_tool_resources_file_search_vector_stores_inner> > (getVectorStores());
		JsonArray* json_array = json_array_new();
		...
		for (list<CreateThreadRequest_tool_resources_file_search_vector_stores_inner>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			CreateThreadRequest_tool_resources_file_search_vector_stores_inner obj = *it;
