...
static gboolean __EmbeddingsManagerresponseHandler(gpointer data)
{
...
static gpointer __EmbeddingsManagerthreadFunc(gpointer data)
{
	RequestInfo *request = static_cast<RequestInfo*>(data);
	...
	g_idle_add(__EmbeddingsManagerresponseHandler, static_cast<gpointer>(request));
	...
	, void* userData, bool isAsync)
{
	...
	if(!isAsync){
		NetClient::easycurl(EmbeddingsManager::getBasePath(), url, myhttpmethod, queryParams,
		...
		requestInfo = new(nothrow) RequestInfo (EmbeddingsManager::getBasePath(), url, myhttpmethod, queryParams,
			mBody, headerList, p_chunk, &code, errormsg, userData, reinterpret_cast<void(*)()>(handler), createEmbeddingProcessor);;
		if(requestInfo == NULL)
			return false;

		thread = g_thread_new(NULL, __EmbeddingsManagerthreadFunc, static_cast<gpointer>(requestInfo));
		return true;
	}
}
...
bool EmbeddingsManager::createEmbeddingAsync(char * accessToken,
	std::shared_ptr<CreateEmbeddingRequest> createEmbeddingRequest, 
	void(* handler)(CreateEmbeddingResponse, Error, void* )
	, void* userData)
{
...
bool EmbeddingsManager::createEmbeddingSync(char * accessToken,
	std::shared_ptr<CreateEmbeddingRequest> createEmbeddingRequest, 
	void(* handler)(CreateEmbeddingResponse, Error, void* )
	, void* userData)
{
