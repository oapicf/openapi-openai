...
void
ListAuditLogsResponse::__cleanup()
{
	//if(object != NULL) {
	//
	//delete object;
...
void
ListAuditLogsResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<AuditLog> new_list;
			AuditLog inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("AuditLog")) {
					jsonToValue(&inst, temp_json, "AuditLog", "");
				} else {
...
ListAuditLogsResponse::ListAuditLogsResponse(char* json)
{
	this->fromJson(json);
}
...
ListAuditLogsResponse::toJson()
{
	...
	if (isprimitive("AuditLog")) {
		list<AuditLog> new_list = static_cast<list <AuditLog> > (getData());
		node = converttoJson(&new_list, "AuditLog", "array");
	} else {
		...
		for (list<AuditLog>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			AuditLog obj = *it;
...
bool
ListAuditLogsResponse::getHasMore()
{
	return has_more;
}
