...
void
ListRunStepsResponse::__cleanup()
{
	//if(object != NULL) {
	//
	//delete object;
...
void
ListRunStepsResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<RunStepObject> new_list;
			RunStepObject inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("RunStepObject")) {
					jsonToValue(&inst, temp_json, "RunStepObject", "");
				} else {
...
ListRunStepsResponse::ListRunStepsResponse(char* json)
{
	this->fromJson(json);
}
...
ListRunStepsResponse::toJson()
{
	...
	if (isprimitive("RunStepObject")) {
		list<RunStepObject> new_list = static_cast<list <RunStepObject> > (getData());
		node = converttoJson(&new_list, "RunStepObject", "array");
	} else {
		...
		for (list<RunStepObject>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			RunStepObject obj = *it;
...
bool
ListRunStepsResponse::getHasMore()
{
	return has_more;
}
