...
void
ListVectorStoreFilesResponse::__init()
{
	//object = std::string();
...
void
ListVectorStoreFilesResponse::__cleanup()
{
	//if(object != NULL) {
	//
	//delete object;
...
void
ListVectorStoreFilesResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<VectorStoreFileObject> new_list;
			VectorStoreFileObject inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("VectorStoreFileObject")) {
					jsonToValue(&inst, temp_json, "VectorStoreFileObject", "");
				} else {
...
ListVectorStoreFilesResponse::ListVectorStoreFilesResponse(char* json)
{
	this->fromJson(json);
}
...
ListVectorStoreFilesResponse::toJson()
{
	...
	if (isprimitive("VectorStoreFileObject")) {
		list<VectorStoreFileObject> new_list = static_cast<list <VectorStoreFileObject> > (getData());
		node = converttoJson(&new_list, "VectorStoreFileObject", "array");
	} else {
		...
		for (list<VectorStoreFileObject>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			VectorStoreFileObject obj = *it;
...
bool
ListVectorStoreFilesResponse::getHasMore()
{
	return has_more;
}
