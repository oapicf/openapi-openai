...
void
ListVectorStoresResponse::__init()
{
	//object = std::string();
...
void
ListVectorStoresResponse::__cleanup()
{
	//if(object != NULL) {
	//
	//delete object;
...
void
ListVectorStoresResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<VectorStoreObject> new_list;
			VectorStoreObject inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("VectorStoreObject")) {
					jsonToValue(&inst, temp_json, "VectorStoreObject", "");
				} else {
...
ListVectorStoresResponse::ListVectorStoresResponse(char* json)
{
	this->fromJson(json);
}
...
ListVectorStoresResponse::toJson()
{
	...
	if (isprimitive("VectorStoreObject")) {
		list<VectorStoreObject> new_list = static_cast<list <VectorStoreObject> > (getData());
		node = converttoJson(&new_list, "VectorStoreObject", "array");
	} else {
		...
		for (list<VectorStoreObject>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			VectorStoreObject obj = *it;
...
bool
ListVectorStoresResponse::getHasMore()
{
	return has_more;
}
