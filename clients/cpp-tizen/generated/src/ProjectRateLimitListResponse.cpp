...
void
ProjectRateLimitListResponse::__init()
{
	//object = std::string();
...
void
ProjectRateLimitListResponse::__cleanup()
{
	//if(object != NULL) {
	//
	//delete object;
...
void
ProjectRateLimitListResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ProjectRateLimit> new_list;
			ProjectRateLimit inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ProjectRateLimit")) {
					jsonToValue(&inst, temp_json, "ProjectRateLimit", "");
				} else {
...
ProjectRateLimitListResponse::ProjectRateLimitListResponse(char* json)
{
	this->fromJson(json);
}
...
ProjectRateLimitListResponse::toJson()
{
	...
	if (isprimitive("ProjectRateLimit")) {
		list<ProjectRateLimit> new_list = static_cast<list <ProjectRateLimit> > (getData());
		node = converttoJson(&new_list, "ProjectRateLimit", "array");
	} else {
		...
		for (list<ProjectRateLimit>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ProjectRateLimit obj = *it;
...
bool
ProjectRateLimitListResponse::getHasMore()
{
	return has_more;
}
