...
void
ProjectServiceAccountListResponse::__init()
{
	//object = std::string();
...
void
ProjectServiceAccountListResponse::__cleanup()
{
	//if(object != NULL) {
	//
	//delete object;
...
void
ProjectServiceAccountListResponse::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ProjectServiceAccount> new_list;
			ProjectServiceAccount inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ProjectServiceAccount")) {
					jsonToValue(&inst, temp_json, "ProjectServiceAccount", "");
				} else {
...
ProjectServiceAccountListResponse::ProjectServiceAccountListResponse(char* json)
{
	this->fromJson(json);
}
...
ProjectServiceAccountListResponse::toJson()
{
	...
	if (isprimitive("ProjectServiceAccount")) {
		list<ProjectServiceAccount> new_list = static_cast<list <ProjectServiceAccount> > (getData());
		node = converttoJson(&new_list, "ProjectServiceAccount", "array");
	} else {
		...
		for (list<ProjectServiceAccount>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ProjectServiceAccount obj = *it;
...
bool
ProjectServiceAccountListResponse::getHasMore()
{
	return has_more;
}
