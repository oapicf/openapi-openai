...
void
RealtimeClientEventSessionUpdate::__init()
{
	//event_id = std::string();
	//type = std::string();
	//session = new RealtimeSessionCreateRequest();
}

void
RealtimeClientEventSessionUpdate::__cleanup()
{
	//if(event_id != NULL) {
	//
	//delete event_id;
...
void
RealtimeClientEventSessionUpdate::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
		...
		if (isprimitive("RealtimeSessionCreateRequest")) {
			jsonToValue(&session, node, "RealtimeSessionCreateRequest", "RealtimeSessionCreateRequest");
		} else {
			
			RealtimeSessionCreateRequest* obj = static_cast<RealtimeSessionCreateRequest*> (&session);
...
RealtimeClientEventSessionUpdate::RealtimeClientEventSessionUpdate(char* json)
{
	this->fromJson(json);
}
...
RealtimeClientEventSessionUpdate::toJson()
{
	...
	if (isprimitive("RealtimeSessionCreateRequest")) {
		RealtimeSessionCreateRequest obj = getSession();
		node = converttoJson(&obj, "RealtimeSessionCreateRequest", "");
	}
	else {
		
		RealtimeSessionCreateRequest obj = static_cast<RealtimeSessionCreateRequest> (getSession());
		GError *mygerror;
		mygerror = NULL;
