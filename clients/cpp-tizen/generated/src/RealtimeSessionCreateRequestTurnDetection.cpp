...
void
RealtimeSessionCreateRequest_turn_detection::__init()
{
	//type = std::string();
	//threshold = double(0);
...
void
RealtimeSessionCreateRequest_turn_detection::__cleanup()
{
	//if(type != NULL) {
	//
	//delete type;
...
void
RealtimeSessionCreateRequest_turn_detection::fromJson(char* jsonStr)
{
	...
	const gchar *create_responseKey = "create_response";
	node = json_object_get_member(pJsonObject, create_responseKey);
	if (node !=NULL) {
	

		if (isprimitive("bool")) {
			jsonToValue(&create_response, node, "bool", "");
		} else {
...
RealtimeSessionCreateRequest_turn_detection::RealtimeSessionCreateRequest_turn_detection(char* json)
{
	this->fromJson(json);
}
...
RealtimeSessionCreateRequest_turn_detection::toJson()
{
	...
	const gchar *create_responseKey = "create_response";
	json_object_set_member(pJsonObject, create_responseKey, node);
...
void
RealtimeSessionCreateRequest_turn_detection::setThreshold(long long  threshold)
{
...
bool
RealtimeSessionCreateRequest_turn_detection::getCreateResponse()
{
