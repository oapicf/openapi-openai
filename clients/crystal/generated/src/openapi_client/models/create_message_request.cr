...
module OpenAPIClient
  class CreateMessageRequest
    ...
    def initialize(@role : String, @content : String, @file_ids : Array(String)? = nil, @metadata : Object? = nil)
    end
    ...
    def valid?
      ...
      return false if @content.try &.to_s.try &.size.try &.> 256000
      return false if @content.try &.to_s.try &.size.try &.< 1
      return false if !@file_ids.nil? && @file_ids.try &.size.try &.> 10
      return false if !@file_ids.nil? && @file_ids.try &.size.try &.< 1
      true
    end
    ...
    def content=(content)
      ...
      if content.to_s.size < 1
        raise ArgumentError.new("invalid value for \"content\", the character length must be greater than or equal to 1.")
      end
