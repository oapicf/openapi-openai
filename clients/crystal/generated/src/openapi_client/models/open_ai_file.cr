...
module OpenAPIClient
  ...
  class OpenAIFile
    ...
    class EnumAttributeValidatorForObject < EnumAttributeValidator
      ...
      def initialize
        @attribute = "object"
        @allowable_values = ["file"].map { |value| to(String, value)}
      end
    end

    class EnumAttributeValidatorForPurpose < EnumAttributeValidator
      @attribute : String
      @allowable_values : Array(Int32 | Int64 | Float32 | Float64 | String)

      def initialize
        @attribute = "purpose"
        @allowable_values = ["assistants", "assistants_output", "batch", "batch_output", "fine-tune", "fine-tune-results", "vision"].map { |value| to(String, value)}
      end
    end
    ...
    class EnumAttributeValidatorForStatus < EnumAttributeValidator
      ...
      def initialize
        @attribute = "status"
        @allowable_values = ["uploaded", "processed", "error"].map { |value| to(String, value)}
      end
    end
    ...
    def initialize(@id : String, @bytes : Int32, @created_at : Int32, @filename : String, @object : String, @purpose : String, @status : String, @status_details : String? = nil)
    end
    ...
    def list_invalid_properties
      ...
      purpose_validator = EnumAttributeValidatorForPurpose.new
      if !purpose_validator.valid?(@purpose)
        message = purpose_validator.message
        invalid_properties.push(message)
      end
    ...
    def purpose=(purpose)
      validator = EnumAttributeValidatorForPurpose.new
