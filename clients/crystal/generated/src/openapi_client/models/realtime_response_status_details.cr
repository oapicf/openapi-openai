...
module OpenAPIClient
  ...
  class RealtimeResponseStatusDetails
    ...
    @[JSON::Field(key: "error", type: RealtimeResponseStatusDetailsError?, nillable: true, emit_null: false)]
    property error : RealtimeResponseStatusDetailsError?
    ...
    class EnumAttributeValidatorForReason < EnumAttributeValidator
      @attribute : String
      @allowable_values : Array(Int32 | Int64 | Float32 | Float64 | String)

      def initialize
        @attribute = "reason"
        @allowable_values = ["turn_detected", "client_cancelled", "max_output_tokens", "content_filter"].map { |value| to(String, value)}
      end
    end
    ...
    def initialize(@_type : String? = nil, @reason : String? = nil, @error : RealtimeResponseStatusDetailsError? = nil)
    end
    ...
    def list_invalid_properties
      ...
      reason_validator = EnumAttributeValidatorForReason.new
      if !reason_validator.valid?(@reason)
        message = reason_validator.message
        invalid_properties.push(message)
      end
    ...
    def reason=(reason)
      validator = EnumAttributeValidatorForReason.new
