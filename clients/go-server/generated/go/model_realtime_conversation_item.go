...
// AssertRealtimeConversationItemRequired checks if the required fields are not zero-ed
func AssertRealtimeConversationItemRequired(obj RealtimeConversationItem) error {
	for _, el := range obj.Content {
		if err := AssertRealtimeConversationItemContentInnerRequired(el); err != nil {
			return err
		}
	}
	return nil
...
// AssertRealtimeConversationItemConstraints checks if the values respects the defined constraints
func AssertRealtimeConversationItemConstraints(obj RealtimeConversationItem) error {
	for _, el := range obj.Content {
		if err := AssertRealtimeConversationItemContentInnerConstraints(el); err != nil {
			return err
		}
	}
	return nil
