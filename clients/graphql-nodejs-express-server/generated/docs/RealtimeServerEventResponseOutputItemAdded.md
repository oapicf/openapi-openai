# RealtimeServerEventResponseOutputItemAdded

## Properties
Name | Type | Description | Notes
------------ | ------------- | ------------- | -------------
**eventId** | **String!** | The unique ID of the server event. | [default to null]
**Type_** | **String!** | The event type, must be &#x60;response.output_item.added&#x60;. | [default to null]
**responseId** | **String!** | The ID of the Response to which the item belongs. | [default to null]
**outputIndex** | **Int!** | The index of the output item in the Response. | [default to null]
**item** | [***RealtimeConversationItem**](RealtimeConversationItem.md) |  | [default to null]

[[Back to Model list]](../README.md#documentation-for-models) [[Back to API list]](../README.md#documentation-for-api-endpoints) [[Back to README]](../README.md)


