...
-- | display the config
instance P.Show OpenAIConfig where
  show c =
    T.printf
...
-- | constructs a default OpenAIConfig
--
-- configHost:
--
...
-- @"openai/0.1.0.0"@
--
newConfig :: IO OpenAIConfig
newConfig = do
    logCxt <- initLogContext
    return $ OpenAIConfig
        { configHost = "https://api.openai.com/v1"
        , configUserAgent = "openai/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => OpenAIConfig -> auth -> OpenAIConfig
addAuthMethod config@OpenAIConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: OpenAIConfig -> IO OpenAIConfig
...
withStderrLogging :: OpenAIConfig -> IO OpenAIConfig
...
withNoLogging :: OpenAIConfig -> OpenAIConfig
...
data OpenAIRequest req contentType res accept = OpenAIRequest
  { rMethod  :: NH.Method   -- ^ Method of OpenAIRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of OpenAIRequest
  , rParams   :: Params -- ^ params of OpenAIRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (OpenAIRequest req contentType res accept) NH.Method
rMethodL f OpenAIRequest{..} = (\rMethod -> OpenAIRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (OpenAIRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f OpenAIRequest{..} = (\rUrlPath -> OpenAIRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (OpenAIRequest req contentType res accept) Params
rParamsL f OpenAIRequest{..} = (\rParams -> OpenAIRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (OpenAIRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f OpenAIRequest{..} = (\rAuthTypes -> OpenAIRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => OpenAIRequest req contentType res accept -> param -> OpenAIRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: OpenAIRequest req contentType res accept -> param -> OpenAIRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: OpenAIRequest req contentType res accept -> param -> OpenAIRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method
          ...
          -> OpenAIRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = OpenAIRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     OpenAIRequest req contentType res accept
  -> [NH.Header]
  -> OpenAIRequest req contentType res accept
...
removeHeader :: OpenAIRequest req contentType res accept -> [NH.HeaderName] -> OpenAIRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => OpenAIRequest req contentType res accept -> OpenAIRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => OpenAIRequest req contentType res accept -> OpenAIRequest req contentType res accept
...
addForm :: OpenAIRequest req contentType res accept -> WH.Form -> OpenAIRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: OpenAIRequest req contentType res accept -> NH.Part -> OpenAIRequest req contentType res accept
...
_setBodyBS :: OpenAIRequest req contentType res accept -> B.ByteString -> OpenAIRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: OpenAIRequest req contentType res accept -> BL.ByteString -> OpenAIRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => OpenAIRequest req contentType res accept -> P.Proxy authMethod -> OpenAIRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> OpenAIRequest req contentType res accept
    -> IO (OpenAIRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> OpenAIConfig
  -> IO (OpenAIRequest req contentType res accept)
_applyAuthMethods req config@(OpenAIConfig {configAuthMethods = as}) =
  foldlM go req as
  where
