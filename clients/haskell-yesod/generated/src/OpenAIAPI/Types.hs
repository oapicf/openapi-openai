...
data CreateFineTuningJobRequestIntegrationsInnerType = CreateFineTuningJobRequestIntegrationsInnerType
  { 
  } deriving (Show, Eq, Generic)

instance FromJSON CreateFineTuningJobRequestIntegrationsInnerType where
  parseJSON = genericParseJSON optionsCreateFineTuningJobRequestIntegrationsInnerType
instance ToJSON CreateFineTuningJobRequestIntegrationsInnerType where
  toJSON = genericToJSON optionsCreateFineTuningJobRequestIntegrationsInnerType

optionsCreateFineTuningJobRequestIntegrationsInnerType :: Options
optionsCreateFineTuningJobRequestIntegrationsInnerType =
  defaultOptions
    { omitNothingFields  = True
    , fieldLabelModifier = \s -> fromMaybe ("did not find JSON field name for " ++ show s) $ List.lookup s table
    }
...
data ThreadStreamEventOneOf = ThreadStreamEventOneOf
  { threadStreamEventOneOfEvent :: Text -- ^ 
  , threadStreamEventOneOfData :: ThreadObject -- ^ 
  } deriving (Show, Eq, Generic)

instance FromJSON ThreadStreamEventOneOf where
  parseJSON = genericParseJSON optionsThreadStreamEventOneOf
instance ToJSON ThreadStreamEventOneOf where
  toJSON = genericToJSON optionsThreadStreamEventOneOf

optionsThreadStreamEventOneOf :: Options
optionsThreadStreamEventOneOf =
  defaultOptions
    { omitNothingFields  = True
    , fieldLabelModifier = \s -> fromMaybe ("did not find JSON field name for " ++ show s) $ List.lookup s table
    }
  where
    table =
      [ ("threadStreamEventOneOfEvent", "event")
      , ("threadStreamEventOneOfData", "data")
      ]
