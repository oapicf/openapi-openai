...
public class ProjectRateLimit {
  ...
  static {
    ...
    openapiFields = new HashSet<String>(Arrays.asList("object", "id", "model", "max_requests_per_1_minute", "max_tokens_per_1_minute", "max_images_per_1_minute", "max_audio_megabytes_per_1_minute", "max_requests_per_1_day", "batch_1_day_max_input_tokens"));

    // a set of required properties/fields (JSON key names)
    openapiRequiredFields = new HashSet<String>(Arrays.asList("object", "id", "model", "max_requests_per_1_minute", "max_tokens_per_1_minute"));
  }
   ...
   * @throws IOException if the JSON Element is invalid with respect to ProjectRateLimit
   */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ProjectRateLimit.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The required field(s) %s in ProjectRateLimit is not found in the empty JSON string", ProjectRateLimit.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ProjectRateLimit.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format(java.util.Locale.ROOT, "The field `%s` in the JSON string is not defined in the `ProjectRateLimit` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ProjectRateLimit.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ProjectRateLimit.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ProjectRateLimit' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ProjectRateLimit> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ProjectRateLimit.class));

       return (TypeAdapter<T>) new TypeAdapter<ProjectRateLimit>() {
           @Override
           public void write(JsonWriter out, ProjectRateLimit value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ProjectRateLimit read(JsonReader in) throws IOException {
  ...
  /**
   * Create an instance of ProjectRateLimit given an JSON string
   *
   * @param jsonString JSON string
   * @return An instance of ProjectRateLimit
   * @throws IOException if the JSON string is invalid with respect to ProjectRateLimit
   */
  public static ProjectRateLimit fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ProjectRateLimit.class);
  }

  /**
   * Convert an instance of ProjectRateLimit to an JSON string
   *
   * @return JSON string
   */
