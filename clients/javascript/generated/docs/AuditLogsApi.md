...
> ListAuditLogsResponse listAuditLogs(opts)
...
let apiInstance = new OpenapiOpenai.AuditLogsApi();
let opts = {
  'effectiveAt': new OpenapiOpenai.ListAuditLogsEffectiveAtParameter(), // ListAuditLogsEffectiveAtParameter | Return only events whose `effective_at` (Unix seconds) is in this range.
  'projectIds': ["null"], // [String] | Return only events for these projects.
  'eventTypes': [new OpenapiOpenai.AuditLogEventType()], // [AuditLogEventType] | Return only events with a `type` in one of these values. For example, `project.created`. For all options, see the documentation for the [audit log object](/docs/api-reference/audit-logs/object).
  'actorIds': ["null"], // [String] | Return only events performed by these actors. Can be a user ID, a service account ID, or an api key tracking ID.
  'actorEmails': ["null"], // [String] | Return only events performed by users with these emails.
  'resourceIds': ["null"], // [String] | Return only events performed on these targets. For example, a project ID updated.
...
apiInstance.listAuditLogs(opts, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
