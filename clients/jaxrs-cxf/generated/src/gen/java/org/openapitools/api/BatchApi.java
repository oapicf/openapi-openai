...
public interface BatchApi  {
    ...
    @ApiOperation(value = "Cancels an in-progress batch. The batch will be in status `cancelling` for up to 10 minutes, before changing to `cancelled`, where it will have partial results (if any) available in the output file.", tags={ "Batch" })
    @ApiResponses(value = { 
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Creates and executes a batch from an uploaded file of requests", tags={ "Batch" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Batch created successfully.", response = Batch.class) })
    public Batch createBatch(@Valid @NotNull CreateBatchRequest createBatchRequest);
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "List your organization's batches.", tags={ "Batch" })
    @ApiResponses(value = { 
    ...
    @Path("/{batch_id}")
    @Produces({ "application/json" })
    @ApiOperation(value = "Retrieves a batch.", tags={ "Batch" })
    @ApiResponses(value = { 
