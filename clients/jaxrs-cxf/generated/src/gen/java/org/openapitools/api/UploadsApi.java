...
public interface UploadsApi  {
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Cancels the Upload. No Parts may be added after an Upload is cancelled. ", tags={ "Uploads" })
    @ApiResponses(value = { 
    ...
    @Produces({ "application/json" })
    @ApiOperation(value = "Completes the [Upload](/docs/api-reference/uploads/object).   Within the returned Upload object, there is a nested [File](/docs/api-reference/files/object) object that is ready to use in the rest of the platform.  You can specify the order of the Parts by passing in an ordered list of the Part IDs.  The number of bytes uploaded upon completion must match the number of bytes initially specified when creating the Upload object. No Parts may be added after an Upload is completed. ", tags={ "Uploads" })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = Upload.class) })
    public Upload completeUpload(@PathParam("upload_id") String uploadId, @Valid @NotNull CompleteUploadRequest completeUploadRequest);
