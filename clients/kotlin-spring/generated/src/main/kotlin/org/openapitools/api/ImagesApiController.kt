...
import org.openapitools.api.ImagesApiController.Companion.BASE_PATH
...
class ImagesApiController() {
    ...
    @RequestMapping(
        ...
        value = [PATH_CREATE_IMAGE /* "/images/generations" */],
        produces = ["application/json"],
        consumes = ["application/json"]
    )
    fun createImage(
        @Parameter(description = "", required = true) @Valid @RequestBody createImageRequest: CreateImageRequest
    ...
    @RequestMapping(
        method = [RequestMethod.POST],
        value = [PATH_CREATE_IMAGE_EDIT /* "/images/edits" */],
        produces = ["application/json"],
        consumes = ["multipart/form-data"]
    )
    fun createImageEdit(
        @Parameter(description = "The image to edit. Must be a valid PNG file, less than 4MB, and square. If mask is not provided, image must have transparency, which will be used as the mask.") @Valid @RequestPart("image", required = true) image: org.springframework.web.multipart.MultipartFile,
        @Parameter(description = "A text description of the desired image(s). The maximum length is 1000 characters.", required = true) @Valid @RequestParam(value = "prompt", required = true) prompt: kotlin.String,
        @Parameter(description = "An additional image whose fully transparent areas (e.g. where alpha is zero) indicate where `image` should be edited. Must be a valid PNG file, less than 4MB, and have the same dimensions as `image`.") @Valid @RequestPart("mask", required = false) mask: org.springframework.web.multipart.MultipartFile,
        @Parameter(description = "") @Valid @RequestPart(value = "model", required = false) model: CreateImageEditRequestModel?,
    ...
    @RequestMapping(
        method = [RequestMethod.POST],
        value = [PATH_CREATE_IMAGE_VARIATION /* "/images/variations" */],
        produces = ["application/json"],
        consumes = ["multipart/form-data"]
    )
    fun createImageVariation(
        @Parameter(description = "The image to use as the basis for the variation(s). Must be a valid PNG file, less than 4MB, and square.") @Valid @RequestPart("image", required = true) image: org.springframework.web.multipart.MultipartFile,
        @Parameter(description = "") @Valid @RequestPart(value = "model", required = false) model: CreateImageEditRequestModel?,
    ...
    companion object {
        ...
        const val PATH_CREATE_IMAGE: String = "/images/generations"
        const val PATH_CREATE_IMAGE_EDIT: String = "/images/edits"
        const val PATH_CREATE_IMAGE_VARIATION: String = "/images/variations"
    }
}
