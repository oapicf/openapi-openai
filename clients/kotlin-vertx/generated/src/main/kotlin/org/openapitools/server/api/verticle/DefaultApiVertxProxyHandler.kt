        ...
        try {
            ...
            when (action) {
                ...
                "adminApiKeysCreate" -> {
                    ...
                    val adminApiKeysCreateRequestParam = ApiHandlerUtils.searchJsonObjectInJson(params,"body")
                    if (adminApiKeysCreateRequestParam == null) {
                        throw IllegalArgumentException("adminApiKeysCreateRequest is required")
                    }
                    val adminApiKeysCreateRequest = Gson().fromJson(adminApiKeysCreateRequestParam.encode(), AdminApiKeysCreateRequest::class.java)
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.adminApiKeysCreate(adminApiKeysCreateRequest,context)
                ...
                "adminApiKeysDelete" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.adminApiKeysDelete(keyId,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "adminApiKeysGet" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.adminApiKeysGet(keyId,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "adminApiKeysList" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.adminApiKeysList(after,order,limit,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
