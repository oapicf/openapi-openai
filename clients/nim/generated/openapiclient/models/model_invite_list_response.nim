...
type InviteListResponse* = object
  ## 
  `object`*: `Object` ## The object type, which is always `list`
  data*: seq[Invite]
  firstId*: Option[string] ## The first `invite_id` in the retrieved `list`
  lastId*: Option[string] ## The last `invite_id` in the retrieved `list`
  hasMore*: Option[bool] ## The `has_more` property is used for pagination to indicate there are additional results.
...
# Custom JSON deserialization for InviteListResponse with custom field names
proc to*(node: JsonNode, T: typedesc[InviteListResponse]): InviteListResponse =
    ...
    if node.hasKey("data"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.data.add(to(item, Invite))
...
# Custom JSON serialization for InviteListResponse with custom field names
proc `%`*(obj: InviteListResponse): JsonNode =
  result = newJObject()
