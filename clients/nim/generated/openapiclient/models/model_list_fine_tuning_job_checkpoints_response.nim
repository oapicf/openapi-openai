...
# Custom JSON deserialization for ListFineTuningJobCheckpointsResponse with custom field names
proc to*(node: JsonNode, T: typedesc[ListFineTuningJobCheckpointsResponse]): ListFineTuningJobCheckpointsResponse =
    ...
    if node.hasKey("data"):
      ...
      if arrayNode.kind == JArray:
        ...
        for item in arrayNode.items:
          result.data.add(to(item, FineTuningJobCheckpoint))
...
# Custom JSON serialization for ListFineTuningJobCheckpointsResponse with custom field names
proc `%`*(obj: ListFineTuningJobCheckpointsResponse): JsonNode =
  result = newJObject()
