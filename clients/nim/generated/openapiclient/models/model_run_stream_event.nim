...
import model_run_stream_event_one_of9
...
type RunStreamEvent* = object
  ...
  of RunStreamEventKind.RunStreamEventOneOf9Variant:
    RunStreamEvent_oneOf_9Value*: RunStreamEventOneOf9
...
proc to*(node: JsonNode, T: typedesc[RunStreamEvent]): RunStreamEvent =
  ...
  try:
    return RunStreamEvent(kind: RunStreamEventKind.RunStreamEventOneOf9Variant, RunStreamEvent_oneOf_9Value: to(node, RunStreamEventOneOf9))
  except Exception as e:
    when defined(debug):
      echo "Failed to deserialize as RunStreamEventOneOf9: ", e.msg
