...
function ConvertFrom-JsonToChatCompletionRequestMessage {
    ...
    Process {
        ...
        # try to match ChatCompletionRequestDeveloperMessage defined in the oneOf schemas
        try {
            $matchInstance = ConvertFrom-JsonToChatCompletionRequestDeveloperMessage $Json

            foreach($property in $matchInstance.PsObject.Properties) {
                if ($null -ne $property.Value) {
                    $matchType = "ChatCompletionRequestDeveloperMessage"
                    $match++
                    break
                }
            }
            ...
            # fail to match the schema defined in oneOf, proceed to the next one
            Write-Debug "Failed to match 'ChatCompletionRequestDeveloperMessage' defined in oneOf (ChatCompletionRequestMessage). Proceeding to the next one if any."
        }
        ...
        if ($match -gt 1) {
            throw "Error! The JSON payload matches more than one type defined in oneOf schemas ([ChatCompletionRequestAssistantMessage, ChatCompletionRequestDeveloperMessage, ChatCompletionRequestFunctionMessage, ChatCompletionRequestSystemMessage, ChatCompletionRequestToolMessage, ChatCompletionRequestUserMessage]). JSON Payload: $($Json)"
        } elseif ($match -eq 1) {
            return [PSCustomObject]@{
                "ActualType" = ${matchType}
                "ActualInstance" = ${matchInstance}
                "OneOfSchemas" = @("ChatCompletionRequestAssistantMessage", "ChatCompletionRequestDeveloperMessage", "ChatCompletionRequestFunctionMessage", "ChatCompletionRequestSystemMessage", "ChatCompletionRequestToolMessage", "ChatCompletionRequestUserMessage")
            }
        } else {
            throw "Error! The JSON payload doesn't matches any type defined in oneOf schemas ([ChatCompletionRequestAssistantMessage, ChatCompletionRequestDeveloperMessage, ChatCompletionRequestFunctionMessage, ChatCompletionRequestSystemMessage, ChatCompletionRequestToolMessage, ChatCompletionRequestUserMessage]). JSON Payload: $($Json)"
        }
    }
}
