...
function Initialize-CreateFineTuningJobRequest {
    ...
    Process {
        ...
        if (!$Suffix -and $Suffix.length -gt 64) {
            throw "invalid value for 'Suffix', the character length must be smaller than or equal to 64."
        }

        if (!$Suffix -and $Suffix.length -lt 1) {
            throw "invalid value for 'Suffix', the character length must be greater than or equal to 1."
        }
...
function ConvertFrom-JsonToCreateFineTuningJobRequest {
    ...
    Process {
        ...
        $AllProperties = ("model", "training_file", "hyperparameters", "suffix", "validation_file", "integrations", "seed", "method")
