#
# OpenAI API
# The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
# Version: 2.3.0
# Contact: blah+oapicf@cliffano.com
# Generated by OpenAPI Generator: https://openapi-generator.tech
#

<#
.SYNOPSIS

No summary available.

.DESCRIPTION

No description available.

.PARAMETER VarData
No description available.
.PARAMETER FirstId
No description available.
.PARAMETER LastId
No description available.
.PARAMETER HasMore
No description available.
.PARAMETER Object
No description available.
.OUTPUTS

ListBatchesResponse<PSCustomObject>
#>

function Initialize-ListBatchesResponse {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [PSCustomObject[]]
        ${VarData},
        [Parameter(Position = 1, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${FirstId},
        [Parameter(Position = 2, ValueFromPipelineByPropertyName = $true)]
        [String]
        ${LastId},
        [Parameter(Position = 3, ValueFromPipelineByPropertyName = $true)]
        [Boolean]
        ${HasMore},
        [Parameter(Position = 4, ValueFromPipelineByPropertyName = $true)]
        [ValidateSet("list")]
        [String]
        ${Object}
    )

    Process {
        'Creating PSCustomObject: PSOpenAPITools => ListBatchesResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        if ($null -eq $VarData) {
            throw "invalid value for 'VarData', 'VarData' cannot be null."
        }

        if ($null -eq $HasMore) {
            throw "invalid value for 'HasMore', 'HasMore' cannot be null."
        }

        if ($null -eq $Object) {
            throw "invalid value for 'Object', 'Object' cannot be null."
        }


        $PSO = [PSCustomObject]@{
            "data" = ${VarData}
            "first_id" = ${FirstId}
            "last_id" = ${LastId}
            "has_more" = ${HasMore}
            "object" = ${Object}
        }


        return $PSO
    }
}

<#
.SYNOPSIS

Convert from JSON to ListBatchesResponse<PSCustomObject>

.DESCRIPTION

Convert from JSON to ListBatchesResponse<PSCustomObject>

.PARAMETER Json

Json object

.OUTPUTS

ListBatchesResponse<PSCustomObject>
#>
function ConvertFrom-JsonToListBatchesResponse {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )

    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => ListBatchesResponse' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in ListBatchesResponse
        $AllProperties = ("data", "first_id", "last_id", "has_more", "object")
        foreach ($name in $JsonParameters.PsObject.Properties.Name) {
            if (!($AllProperties.Contains($name))) {
                throw "Error! JSON key '$name' not found in the properties: $($AllProperties)"
            }
        }

        If ([string]::IsNullOrEmpty($Json) -or $Json -eq "{}") { # empty json
            throw "Error! Empty JSON cannot be serialized due to the required property 'data' missing."
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "data"))) {
            throw "Error! JSON cannot be serialized due to the required property 'data' missing."
        } else {
            $VarData = $JsonParameters.PSobject.Properties["data"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "has_more"))) {
            throw "Error! JSON cannot be serialized due to the required property 'has_more' missing."
        } else {
            $HasMore = $JsonParameters.PSobject.Properties["has_more"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "object"))) {
            throw "Error! JSON cannot be serialized due to the required property 'object' missing."
        } else {
            $Object = $JsonParameters.PSobject.Properties["object"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "first_id"))) { #optional property not found
            $FirstId = $null
        } else {
            $FirstId = $JsonParameters.PSobject.Properties["first_id"].value
        }

        if (!([bool]($JsonParameters.PSobject.Properties.name -match "last_id"))) { #optional property not found
            $LastId = $null
        } else {
            $LastId = $JsonParameters.PSobject.Properties["last_id"].value
        }

        $PSO = [PSCustomObject]@{
            "data" = ${VarData}
            "first_id" = ${FirstId}
            "last_id" = ${LastId}
            "has_more" = ${HasMore}
            "object" = ${Object}
        }

        return $PSO
    }

}

