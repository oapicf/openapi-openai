...
UsageAudioSpeechesResult<PSCustomObject>
#>

function Initialize-UsageAudioSpeechesResult {
    [CmdletBinding()]
    Param (
        [Parameter(Position = 0, ValueFromPipelineByPropertyName = $true)]
        [ValidateSet("organization.usage.audio_speeches.result")]
        [String]
        ${Object},
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => UsageAudioSpeechesResult' | Write-Debug
        ...
        if ($null -eq $Characters) {
            throw "invalid value for 'Characters', 'Characters' cannot be null."
        }
...
Convert from JSON to UsageAudioSpeechesResult<PSCustomObject>
...
UsageAudioSpeechesResult<PSCustomObject>
#>
function ConvertFrom-JsonToUsageAudioSpeechesResult {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => UsageAudioSpeechesResult' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug

        $JsonParameters = ConvertFrom-Json -InputObject $Json

        # check if Json contains properties not defined in UsageAudioSpeechesResult
        $AllProperties = ("object", "characters", "num_model_requests", "project_id", "user_id", "api_key_id", "model")
        ...
        if (!([bool]($JsonParameters.PSobject.Properties.name -match "characters"))) {
            throw "Error! JSON cannot be serialized due to the required property 'characters' missing."
        } else {
            $Characters = $JsonParameters.PSobject.Properties["characters"].value
        }
