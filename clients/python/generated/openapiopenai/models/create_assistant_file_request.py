...
class CreateAssistantFileRequest(BaseModel):
    ...
    """ # noqa: E501
    file_id: StrictStr = Field(description="A [File](/docs/api-reference/files) ID (with `purpose=\"assistants\"`) that the assistant should use. Useful for tools like `retrieval` and `code_interpreter` that can access files.")
