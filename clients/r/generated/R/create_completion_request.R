...
#' @field suffix The suffix that comes after a completion of inserted text. character [optional]
#' @field max_tokens The maximum number of [tokens](/tokenizer) to generate in the completion.  The token count of your prompt plus `max_tokens` cannot exceed the model's context length. [Example Python code](https://github.com/openai/openai-cookbook/blob/main/examples/How_to_count_tokens_with_tiktoken.ipynb) for counting tokens. integer [optional]
...
#' @field logit_bias Modify the likelihood of specified tokens appearing in the completion.  Accepts a json object that maps tokens (specified by their token ID in the GPT tokenizer) to an associated bias value from -100 to 100. You can use this [tokenizer tool](/tokenizer?view=bpe) (which works for both GPT-2 and GPT-3) to convert text to token IDs. Mathematically, the bias is added to the logits generated by the model prior to sampling. The exact effect will vary per model, but values between -1 and 1 should decrease or increase likelihood of selection; values like -100 or 100 should result in a ban or exclusive selection of the relevant token.  As an example, you can pass `{\"50256\": -100}` to prevent the <|endoftext|> token from being generated. object [optional]
...
CreateCompletionRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @param max_tokens The maximum number of [tokens](/tokenizer) to generate in the completion.  The token count of your prompt plus `max_tokens` cannot exceed the model's context length. [Example Python code](https://github.com/openai/openai-cookbook/blob/main/examples/How_to_count_tokens_with_tiktoken.ipynb) for counting tokens.. Default to 16.
    ...
    #' @export
    initialize = function(`model`, `prompt`, `suffix` = NULL, `max_tokens` = 16, `temperature` = 1, `top_p` = 1, `n` = 1, `stream` = FALSE, `logprobs` = NULL, `echo` = FALSE, `stop` = NULL, `presence_penalty` = 0, `frequency_penalty` = 0, `best_of` = 1, `logit_bias` = NULL, `user` = NULL, ...) {
      if (!missing(`model`)) {
