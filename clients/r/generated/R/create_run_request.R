    ...
    toJSON = function() {
      ...
      if (!is.null(self$`additional_messages`)) {
        CreateRunRequestObject[["additional_messages"]] <-
          lapply(self$`additional_messages`, function(x) x$toJSON())
      }
    ...
    toJSONString = function() {
      jsoncontent <- c(
        ...
        if (!is.null(self$`additional_messages`)) {
          ...
          [%s]
',
          paste(sapply(self$`additional_messages`, function(x) jsonlite::toJSON(x$toJSON(), auto_unbox = TRUE, digits = NA)), collapse = ",")
          )
        },
        if (!is.null(self$`tools`)) {
          sprintf(
...
',
    ...
    #' Validate JSON input with respect to CreateRunRequest
    #'
    #' @description
