...
#' @description CreateRunRequest Class
...
#' @field instructions Overrides the [instructions](/docs/api-reference/assistants/createAssistant) of the assistant. This is useful for modifying the behavior on a per-run basis. character [optional]
#' @field additional_instructions Appends additional instructions at the end of the instructions for the run. This is useful for modifying the behavior on a per-run basis without overriding other instructions. character [optional]
#' @field additional_messages Adds additional messages to the thread before creating the run. list(\link{CreateMessageRequest}) [optional]
#' @field tools Override the tools the assistant can use for this run. This is useful for modifying the behavior on a per-run basis. list(\link{AssistantObjectToolsInner}) [optional]
...
CreateRunRequest <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new CreateRunRequest class.
    #'
    ...
    #' @param additional_instructions Appends additional instructions at the end of the instructions for the run. This is useful for modifying the behavior on a per-run basis without overriding other instructions.
    #' @param additional_messages Adds additional messages to the thread before creating the run.
    ...
    initialize = function(`assistant_id`, `model` = NULL, `instructions` = NULL, `additional_instructions` = NULL, `additional_messages` = NULL, `tools` = NULL, `metadata` = NULL, `temperature` = 1, `stream` = NULL, `max_prompt_tokens` = NULL, `max_completion_tokens` = NULL, `truncation_strategy` = NULL, `tool_choice` = NULL, `response_format` = NULL, ...) {
      ...
      if (!is.null(`additional_instructions`)) {
        if (!(is.character(`additional_instructions`) && length(`additional_instructions`) == 1)) {
          stop(paste("Error! Invalid data for `additional_instructions`. Must be a string:", `additional_instructions`))
        }
        self$`additional_instructions` <- `additional_instructions`
      }
      if (!is.null(`additional_messages`)) {
        stopifnot(is.vector(`additional_messages`), length(`additional_messages`) != 0)
        sapply(`additional_messages`, function(x) stopifnot(R6::is.R6(x)))
    ...
    #'
    #' @return CreateRunRequest as a base R list.
    #' @examples
    #' # convert array of CreateRunRequest (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CreateRunRequest to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      CreateRunRequestObject <- list()
      if (!is.null(self$`assistant_id`)) {
        CreateRunRequestObject[["assistant_id"]] <-
          self$`assistant_id`
      }
      if (!is.null(self$`model`)) {
        CreateRunRequestObject[["model"]] <-
          self$`model`$toSimpleType()
      }
      if (!is.null(self$`instructions`)) {
        CreateRunRequestObject[["instructions"]] <-
          self$`instructions`
      }
      if (!is.null(self$`additional_instructions`)) {
        CreateRunRequestObject[["additional_instructions"]] <-
          self$`additional_instructions`
      }
      if (!is.null(self$`additional_messages`)) {
        CreateRunRequestObject[["additional_messages"]] <-
          lapply(self$`additional_messages`, function(x) x$toSimpleType())
      }
      if (!is.null(self$`tools`)) {
        CreateRunRequestObject[["tools"]] <-
          lapply(self$`tools`, function(x) x$toSimpleType())
      }
      if (!is.null(self$`metadata`)) {
        CreateRunRequestObject[["metadata"]] <-
          self$`metadata`
      }
      if (!is.null(self$`temperature`)) {
        CreateRunRequestObject[["temperature"]] <-
          self$`temperature`
      }
      if (!is.null(self$`stream`)) {
        CreateRunRequestObject[["stream"]] <-
          self$`stream`
      }
      if (!is.null(self$`max_prompt_tokens`)) {
        CreateRunRequestObject[["max_prompt_tokens"]] <-
          self$`max_prompt_tokens`
      }
      if (!is.null(self$`max_completion_tokens`)) {
        CreateRunRequestObject[["max_completion_tokens"]] <-
          self$`max_completion_tokens`
      }
      if (!is.null(self$`truncation_strategy`)) {
        CreateRunRequestObject[["truncation_strategy"]] <-
          self$`truncation_strategy`$toSimpleType()
      }
      if (!is.null(self$`tool_choice`)) {
        CreateRunRequestObject[["tool_choice"]] <-
          self$`tool_choice`$toSimpleType()
      }
      if (!is.null(self$`response_format`)) {
        CreateRunRequestObject[["response_format"]] <-
          self$`response_format`$toSimpleType()
      }
      return(CreateRunRequestObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of CreateRunRequest
    #'
    #' @param input_json the JSON input
    #' @return the instance of CreateRunRequest
    fromJSON = function(input_json) {
      ...
      if (!is.null(this_object$`additional_instructions`)) {
        self$`additional_instructions` <- this_object$`additional_instructions`
      }
      if (!is.null(this_object$`additional_messages`)) {
        self$`additional_messages` <- ApiClient$new()$deserializeObj(this_object$`additional_messages`, "array[CreateMessageRequest]", loadNamespace("openapi"))
      }
    ...
    #' @return CreateRunRequest in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to CreateRunRequest and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`assistant_id`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for CreateRunRequest: the required field `assistant_id` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of CreateRunRequest
    toString = function() {
      self$toJSONString()
    },
...
# CreateRunRequest$unlock()
#
## Below is an example to define the print function
# CreateRunRequest$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# CreateRunRequest$lock()
