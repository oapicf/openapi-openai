...
CreateTranscriptionResponseVerboseJson <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return CreateTranscriptionResponseVerboseJson as a base R list.
    #' @examples
    #' # convert array of CreateTranscriptionResponseVerboseJson (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert CreateTranscriptionResponseVerboseJson to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`words`)) {
        CreateTranscriptionResponseVerboseJsonObject[["words"]] <-
          lapply(self$`words`, function(x) x$toSimpleType())
      }
      if (!is.null(self$`segments`)) {
        CreateTranscriptionResponseVerboseJsonObject[["segments"]] <-
          lapply(self$`segments`, function(x) x$toSimpleType())
      }
      return(CreateTranscriptionResponseVerboseJsonObject)
    },
