#' Create a new RunCompletionUsage
#'
#' @description
...
#' @description RunCompletionUsage Class
#' @format An \code{R6Class} generator object
#' @field completion_tokens Number of completion tokens used over the course of the run. integer
#' @field prompt_tokens Number of prompt tokens used over the course of the run. integer
#' @field total_tokens Total number of tokens used (prompt + completion). integer
#' @importFrom R6 R6Class
#' @importFrom jsonlite fromJSON toJSON
#' @export
RunCompletionUsage <- R6::R6Class(
  "RunCompletionUsage",
  public = list(
    ...
    #' @description
    #' Initialize a new RunCompletionUsage class.
    #'
    #' @param completion_tokens Number of completion tokens used over the course of the run.
    #' @param prompt_tokens Number of prompt tokens used over the course of the run.
    ...
    #'
    #' @return RunCompletionUsage as a base R list.
    #' @examples
    #' # convert array of RunCompletionUsage (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert RunCompletionUsage to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      RunCompletionUsageObject <- list()
      if (!is.null(self$`completion_tokens`)) {
        RunCompletionUsageObject[["completion_tokens"]] <-
          self$`completion_tokens`
      }
      if (!is.null(self$`prompt_tokens`)) {
        RunCompletionUsageObject[["prompt_tokens"]] <-
          self$`prompt_tokens`
      }
      if (!is.null(self$`total_tokens`)) {
        RunCompletionUsageObject[["total_tokens"]] <-
          self$`total_tokens`
      }
      return(RunCompletionUsageObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of RunCompletionUsage
    #'
    #' @param input_json the JSON input
    #' @return the instance of RunCompletionUsage
    ...
    #' @return RunCompletionUsage in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to RunCompletionUsage and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`completion_tokens`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RunCompletionUsage: the required field `completion_tokens` is missing."))
      }
      ...
      if (!is.null(input_json$`prompt_tokens`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RunCompletionUsage: the required field `prompt_tokens` is missing."))
      }
      ...
      if (!is.null(input_json$`total_tokens`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for RunCompletionUsage: the required field `total_tokens` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of RunCompletionUsage
    toString = function() {
      self$toJSONString()
    },
...
# RunCompletionUsage$unlock()
#
## Below is an example to define the print function
# RunCompletionUsage$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# RunCompletionUsage$lock()
