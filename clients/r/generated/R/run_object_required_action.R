...
RunObjectRequiredAction <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return RunObjectRequiredAction as a base R list.
    #' @examples
    #' # convert array of RunObjectRequiredAction (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert RunObjectRequiredAction to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`submit_tool_outputs`)) {
        RunObjectRequiredActionObject[["submit_tool_outputs"]] <-
          self$`submit_tool_outputs`$toSimpleType()
      }
      return(RunObjectRequiredActionObject)
    },
