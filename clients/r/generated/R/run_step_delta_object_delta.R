...
RunStepDeltaObjectDelta <- R6::R6Class(
  ...
  public = list(
    ...
    #'
    #' @return RunStepDeltaObjectDelta as a base R list.
    #' @examples
    #' # convert array of RunStepDeltaObjectDelta (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert RunStepDeltaObjectDelta to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`step_details`)) {
        RunStepDeltaObjectDeltaObject[["step_details"]] <-
          self$`step_details`$toSimpleType()
      }
      return(RunStepDeltaObjectDeltaObject)
    },
