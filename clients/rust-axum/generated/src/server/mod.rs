...
where
    ...
    A: apis::assistants::Assistants<E, Claims = C> + apis::audio::Audio<E, Claims = C> + apis::chat::Chat<E, Claims = C> + apis::completions::Completions<E, Claims = C> + apis::embeddings::Embeddings<E, Claims = C> + apis::files::Files<E, Claims = C> + apis::fine_tuning::FineTuning<E, Claims = C> + apis::images::Images<E, Claims = C> + apis::models::Models<E, Claims = C> + apis::moderations::Moderations<E, Claims = C> + apis::ApiAuthBasic<Claims = C> + Send + Sync + 'static,
    ...
    C: Send + Sync + 'static,
{
    ...
    Router::new()
        ...
        .route("/v1/assistants/{assistant_id}/files",
            get(list_assistant_files::<I, A, E, C>).post(create_assistant_file::<I, A, E, C>)
        )
        .route("/v1/assistants/{assistant_id}/files/{file_id}",
            delete(delete_assistant_file::<I, A, E, C>).get(get_assistant_file::<I, A, E, C>)
        )
...
#[tracing::instrument(skip_all)]
async fn create_assistant_file<I, A, E, C>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  headers: HeaderMap,
...
let result = api_impl.as_ref().create_assistant_file(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::assistants::CreateAssistantFileResponse::Status200_OK
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
...
#[tracing::instrument(skip_all)]
async fn delete_assistant_file<I, A, E, C>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  headers: HeaderMap,
...
let result = api_impl.as_ref().delete_assistant_file(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::assistants::DeleteAssistantFileResponse::Status200_OK
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
...
#[tracing::instrument(skip_all)]
async fn get_assistant_file<I, A, E, C>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  headers: HeaderMap,
...
let result = api_impl.as_ref().get_assistant_file(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::assistants::GetAssistantFileResponse::Status200_OK
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
...
#[tracing::instrument(skip_all)]
async fn get_message_file<I, A, E, C>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  headers: HeaderMap,
...
let result = api_impl.as_ref().get_message_file(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::assistants::GetMessageFileResponse::Status200_OK
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
...
#[tracing::instrument(skip_all)]
async fn list_assistant_files<I, A, E, C>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  headers: HeaderMap,
  Path(path_params): Path<models::ListAssistantFilesPathParams>,
  QueryExtra(query_params): QueryExtra<models::ListAssistantFilesQueryParams>,
 State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
...
let result = api_impl.as_ref().list_assistant_files(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::assistants::ListAssistantFilesResponse::Status200_OK
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
...
#[tracing::instrument(skip_all)]
async fn list_message_files<I, A, E, C>(
  method: Method,
  host: Host,
  cookies: CookieJar,
  headers: HeaderMap,
  Path(path_params): Path<models::ListMessageFilesPathParams>,
  QueryExtra(query_params): QueryExtra<models::ListMessageFilesQueryParams>,
 State(api_impl): State<I>,
) -> Result<Response, StatusCode>
where
    I: AsRef<A> + Send + Sync,
...
let result = api_impl.as_ref().list_message_files(
  ...
  let resp = match result {
                                            Ok(rsp) => match rsp {
                                                apis::assistants::ListMessageFilesResponse::Status200_OK
                                                    (body)
                                                => {
                                                  let mut response = response.status(200);
                                                  {
