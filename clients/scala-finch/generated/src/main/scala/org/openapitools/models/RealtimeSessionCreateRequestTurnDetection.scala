 ...
 * @param createUnderscoreresponse Whether or not to automatically generate a response when VAD is enabled. `true` by default. 
 */
case class RealtimeSessionCreateRequestTurnDetection(_type: Option[String],
                ...
                createUnderscoreresponse: Option[Boolean]
                )

object RealtimeSessionCreateRequestTurnDetection {
    /**
     * Creates the codec for converting RealtimeSessionCreateRequestTurnDetection from and to JSON.
     */
    implicit val decoder: Decoder[RealtimeSessionCreateRequestTurnDetection] = deriveDecoder
    implicit val encoder: ObjectEncoder[RealtimeSessionCreateRequestTurnDetection] = deriveEncoder
}
