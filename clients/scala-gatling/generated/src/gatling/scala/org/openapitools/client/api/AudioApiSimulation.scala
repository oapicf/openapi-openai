...
class AudioApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val createSpeechPerSecond = config.getDouble("performance.operationsPerSecond.createSpeech") * rateMultiplier * instanceMultiplier
    ...
    val scncreateSpeech = scenario("createSpeechSimulation")
        .exec(http("createSpeech")
        .httpRequest("POST","/audio/speech")
)

    // Run scncreateSpeech with warm up and reach a constant rate for entire duration
    scenarioBuilders += scncreateSpeech.inject(
        rampUsersPerSec(1) to(createSpeechPerSecond) during(rampUpSeconds),
        constantUsersPerSec(createSpeechPerSecond) during(durationSeconds),
        rampUsersPerSec(createSpeechPerSecond) to(1) during(rampDownSeconds)
    )
