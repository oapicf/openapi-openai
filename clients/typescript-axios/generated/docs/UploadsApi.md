...
let uploadId: string; //The ID of the Upload.  (default to undefined)
let data: File; //The chunk of bytes for this Part.  (default to undefined)

const { status, data } = await apiInstance.addUploadPart(
    uploadId,
    data
);
```
...
| **data** | [**File**] | The chunk of bytes for this Part.  | defaults to undefined|
...
const { status, data } = await apiInstance.cancelUpload(
    uploadId
);
```
...
let uploadId: string; //The ID of the Upload.  (default to undefined)
let completeUploadRequest: CompleteUploadRequest; //
...
let createUploadRequest: CreateUploadRequest; //
